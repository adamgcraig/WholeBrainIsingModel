...
with torch.no_grad():
    ...
    if homogenize_h:
        ...
        for node_index in h_node_order:
            ...
            num_hom += 1
            modified_sim_file_fragment = f'{original_sim_file_fragment}_hom_{num_hom}_h_nodes_last_{node_index}'
            print(f'time {time.time()-code_start_time:.3f}, testing model with {num_hom} homogenized nodes, most recently {node_index}, priority {h_priorities[node_index]:.3g}, h mean {mean_h:.3g}, SD {std_h:.3g}...')
    ...
    if homogenize_J:
        ...
        for pair_index in J_node_order:
            ...
            num_hom += 1
            modified_sim_file_fragment = f'{original_sim_file_fragment}_hom_{num_hom}_J_pairs_last_{pair_index}'
            print(f'time {time.time()-code_start_time:.3f}, testing model with {num_hom} homogenized node pairs, most recently {pair_index} ({node_i}, {node_j}) priority {J_priorities[pair_index]:.3g}, J mean {mean_J:.3g}, SD {std_J:.3g}...')
            test_model_and_save(model=model, sim_file_fragment=modified_sim_file_fragment, target_state_mean=target_state_mean, target_state_product_mean=target_state_product_mean, target_cov=target_cov, target_fc=target_fc)
