...
def load_data_ts_z():
    ...
    data_ts_all_as_is = torch.load(f=data_ts_file, weights_only=False)[:,:670,:,:]
    print( f'time {time.time()-code_start_time:.3f} loaded {data_ts_file}, size', data_ts_all_as_is.size() )
    data_ts_all_as_is = torch.permute( input=data_ts_all_as_is, dims=(2,1,0,3) ).flatten(start_dim=-3, end_dim=-1)
    ...
    # Clone in case this is still a view of a larger time series.
...
with torch.no_grad():
    ...
    print(f'The maximum binarization threshold at which no region of the 670-subject group time series is all -1 is {data_ts_min_max_z:3g}.')
    print('The rest of the maximum thresholds for regions:')
