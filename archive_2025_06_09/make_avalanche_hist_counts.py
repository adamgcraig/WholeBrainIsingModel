...
with torch.no_grad():
    ...
    parser.add_argument("-e", "--num_thresholds", type=int, default=10, help="number of binarization thresholds in std. dev.s above the mean to try")
    ...
    avalanche_counts = get_avalanche_counts( data_ts=data_ts, threshold=data_ts.median(dim=-1, keepdim=True).values )
    avalanche_counts_file = os.path.join(output_directory, f'avalanche_counts_{data_subset}_{file_name_fragment}_binarized_median.pt')
    ...
    for choice_index in range(num_choices):
        ...
        avalanche_counts[choice_index,:] = get_avalanche_counts( data_ts=data_ts, threshold=choice*data_ts_std )
    avalanche_counts_file = os.path.join(output_directory, f'avalanche_counts_{data_subset}_{file_name_fragment}_choices_{num_choices}_from_{min_threshold:.3g}_to_{max_threshold:.3g}.pt')
    torch.save(obj=avalanche_counts, f=avalanche_counts_file)
    print(f'time {time.time()-code_start_time:.3f}, saved {avalanche_counts_file}')
