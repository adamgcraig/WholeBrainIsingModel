...
hidden_layer_widths = torch.tensor(data=[10, 100, 1000], dtype=int_type, device=device)
...
def train_mlp(sc_and_features_train:torch.Tensor, J_and_h_train:torch.Tensor, sc_and_features_validate:torch.Tensor, J_and_h_validate:torch.Tensor, num_hidden_layers:int=10, num_hidden_features:int=10, batch_size:int=670, learning_rate:float=0.001):
    ...
    num_samples, num_structural_features = sc_and_features_train.size()
    num_ising_params = J_and_h_train.size(dim=-1)
    ...
    model = get_rectangular_mlp(in_features=num_structural_features, out_features=num_ising_params, num_hidden_layers=num_hidden_layers, hidden_layer_width=num_hidden_features, dtype=model_dtype, device=model_device)
...
def save_mlp_results(sc_and_features:torch.Tensor, sc_and_features_name:str, J_and_h:torch.Tensor, J_and_h_name:str):
    ...
    best_mlp_file_part = f'{sc_and_features_name}_{J_and_h_name}_mlp_more_all_all_hidden_num_{best_num_hidden_layers}_width_{best_hidden_layer_width}_batch_{best_batch_size}_lr_{best_learning_rate:.3g}_perms_{num_permutations}_{output_file_name_part}'
...
def get_sc_and_features(num_region_features:int=4):
    ...
    region_features = torch.flatten( input=torch.load(f=region_feature_file_name, weights_only=False)[:,:,:num_region_features], start_dim=-2, end_dim=-1 )
    ...
    sc = torch.load(f=region_pair_feature_file_name, weights_only=False)[:,:,0]
...
def get_J_and_h():
    ...
    J_and_h = torch.mean(  input=torch.cat( tensors=(model.J[:,:,triu_rows,triu_cols], model.h), dim=-1 ), dim=0  )
...
def get_fc_and_mean():
    ...
    num_nodes = fc.size(dim=-1)
    triu_rows, triu_cols = isingmodellight.get_triu_indices_for_products(num_nodes=num_nodes, device=fc.device)
    fc_and_mean = torch.cat( tensors=(fc[:,triu_rows,triu_cols], mean_state), dim=-1 )
    return fc_and_mean
