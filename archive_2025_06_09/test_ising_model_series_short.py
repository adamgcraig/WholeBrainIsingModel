...
with torch.no_grad():
    ...
    parser.add_argument("-a", "--data_directory", type=str, default='E:\\Ising_model_results_daai', help="directory where we can find the target mean state and mean state product files")
    parser.add_argument("-b", "--model_directory", type=str, default='E:\\Ising_model_results_daai', help="directory where we can find the fitted model files")
    parser.add_argument("-c", "--result_directory", type=str, default='E:\\Ising_model_results_daai', help="directory to which we write the results of the testing")
    parser.add_argument("-d", "--mean_state_file_name", type=str, default='mean_state_aal_short_threshold_1.pt', help="name of the target mean state file")
    parser.add_argument("-e", "--mean_state_product_file_name", type=str, default='mean_state_product_aal_short_threshold_1.pt', help="name of the target mean state product file")
    parser.add_argument("-f", "--model_file_prefix", type=str, default='ising_model_aal_short_threshold_1_beta_updates_9_param_updates', help="name of the Ising model file up to the number of parameter updates")
    parser.add_argument("-g", "--update_increment", type=int, default=1000, help="amount by which to increment the number of updates")
    parser.add_argument("-i", "--min_param_updates", type=int, default=1000, help="first number of parameter updates to append")
    parser.add_argument("-j", "--max_param_updates", type=int, default=29000, help="last number of parameter updates to append")
    parser.add_argument("-k", "--sim_length", type=int, default=120000, help="number of simulation steps in the test")
    parser.add_argument("-l", "--save_full_ts", action='store_true', default=False, help="Set this flag to save the entire simulation time series.")
    parser.add_argument("-m", "--beta_test", action='store_true', default=False, help="Set this flag to try a range of beta values across replicas.")
    parser.add_argument("-n", "--min_beta", type=float, default=10e-10, help="low end of beta interval")
    parser.add_argument("-o", "--max_beta", type=float, default=1.0, help="high end of beta interval")
    ...
    model_file_prefix = args.model_file_prefix
    print(f'model_file_prefix={model_file_prefix}')
    update_increment = args.update_increment
    print(f'update_increment={update_increment}')
    min_param_updates = args.min_param_updates
    print(f'min_param_updates={min_param_updates}')
    max_param_updates = args.max_param_updates
    print(f'max_param_updates={max_param_updates}')
    ...
    update_counts = torch.arange(start=min_param_updates, end=max_param_updates+1, step=update_increment, dtype=int_type, device=device)
    num_models = update_counts.numel()
    for update_index in range(num_models):
        update_count = update_counts[update_index]
        model_file_path = os.path.join(model_directory, f'{model_file_prefix}_{update_count}.pt')
        ...
        results_file_suffix = f'test_length_{sim_length}_{model_file_prefix}_{update_count}'
        ...
        cov_corr = isingmodelshort.get_pairwise_correlation_ut(mat1=sim_cov, mat2=target_cov)
        print(f'covariance correlation min {cov_corr.min():.3g}, mean {cov_corr.mean():.3g}, max {cov_corr.max():.3g}')
        cov_corr_file = os.path.join(result_directory, f'cov_corr_{results_file_suffix}')
