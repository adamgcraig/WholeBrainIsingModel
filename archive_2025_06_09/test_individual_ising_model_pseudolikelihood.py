...
with torch.no_grad():
    ...
    parser.add_argument("-q", "--param_file_fragment", type=int, default=120000, help="length of simulations to use in beta test")
    ...
    h_file = os.path.join(output_directory, f'h_{param_file_fragment}.pt')
    h = torch.load(h_file)
    print( f'time {time.time()-code_start_time:.3f}, loaded {h_file}, size', h.size() )
    J_file = os.path.join(output_directory, f'J_{param_file_fragment}.pt')
    J = torch.load(J_file)
    print( f'time {time.time()-code_start_time:.3f}, loaded {J_file}, size', J.size() )
