...
parser = argparse.ArgumentParser(description="Run a batch of Ising models, and compare the time series FC matrices to the real data FC.")
...
parser.add_argument("-p", "--ising_param_string", type=str, default='_parallel_nodes_21_epochs_1000_reps_10_window_50_lr_0.001_threshold_0.100_beta_0.500', help="parameter string of the Ising model file, the characters between h_/J_ and _[subject ID]")
...
parser.add_argument("-d", "--data_subset", type=str, default='all', help="if we want multiple subjects, set it to training or validation")
parser.add_argument("-e", "--expected_num_models", type=str, default=10, help="expected number of models per file")
...
print(f'expected_num_models={expected_num_models}')
...
with torch.no_grad():
    ...
    fc_rmse_set = torch.zeros( (num_subjects, expected_num_models), dtype=float_type, device=device )
    fc_corr_set = torch.zeros( (num_subjects, expected_num_models), dtype=float_type, device=device )
    for subject_index in range(num_subjects):
        ...
        for batch in range(num_batches):
            ...
            print('running sim...')
            current_sim_ts, s = ising.run_batched_balanced_metropolis_sim(sim_ts=current_sim_ts, J=J_batch, h=h_batch, s=s, num_steps=sim_length, beta=beta)
    ...
    rmse_file = os.path.join(output_directory, f'fc_rmse_{ising_param_string}_length_{sim_length}_{subject_string}.pt')
    torch.save(fc_rmse_set, rmse_file)
    print(rmse_file)
    corr_file = os.path.join(output_directory, f'fc_corr_{ising_param_string}_length_{sim_length}_{subject_string}.pt')
    print(corr_file)
