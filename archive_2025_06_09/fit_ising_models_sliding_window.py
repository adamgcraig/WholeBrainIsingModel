...
parser.add_argument("-w", "--window_length", type=int, default=50, help="steps between updates to model parameters")
...
with torch.no_grad():
    ...
    for rep in range(num_reps):    # Initialize the model and run a sim prior to training to establish a baseline for improvement.
        ...
        num_steps = 0
        data_mu_window = torch.zeros( (window_length, num_subjects, num_nodes), dtype=float_type, device=device )
        data_cov_window = torch.zeros( (window_length, num_subjects, num_nodes, num_nodes), dtype=float_type, device=device )
        sim_mu_window = torch.zeros( (window_length, num_subjects, num_nodes), dtype=float_type, device=device )
        sim_cov_window = torch.zeros( (window_length, num_subjects, num_nodes, num_nodes), dtype=float_type, device=device )
        for epoch in range(num_epochs):
            for t in range(num_time_points):
                ...
                sim_ts[:,t,:] = s
                d = data_ts[:,t,:]
                t_in_window = t % window_length
                data_mu_window[t_in_window,:,:] = d
                sim_mu_window[t_in_window,:,:] = s
                data_cov_window[t_in_window,:,:,:] = d[:,:,None] * d[:,None,:]
                sim_cov_window[t_in_window,:,:,:] = s[:,:,None] * s[:,None,:]
                num_steps += 1
                learning_rate_scaled = learning_rate/float(num_steps)
                num_steps = 0
                h += learning_rate_scaled * (data_mu_window - sim_mu_window)
                data_mu_window[:,:] = 0.0
                sim_mu_window[:,:] = 0.0
                J += learning_rate_scaled * (data_cov_window - sim_cov_window)
                data_cov_window[:,:,:] = 0.0
                sim_cov_window[:,:,:] = 0.0
        ...
        file_suffix = f'data_{data_subset}_nodes_{num_nodes}_rep_{rep}_epochs_{num_epochs}_sliding_window_{window_length}_lr_{learning_rate}_threshold_{threshold}_start_{subjects_start}_end_{subjects_end}'
