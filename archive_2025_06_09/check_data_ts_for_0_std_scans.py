...
with torch.no_grad():
    ...
    print(f'The maximum binarization threshold at which no region of any single-scan time series is all -1 is {data_ts_min_max_z:3g}.')
    print('The rest of the maximum thresholds for individual scans:')
    print( data_ts_max_z.tolist() )
    # print(f'time\tthreshold\tnum_all_low\tnum_all_high')
    # for threshold in torch.arange(start=0.0, end=6.0, step=0.01):
