...
with torch.no_grad():
    ...
    for rep in range(num_reps):    # Initialize the model and run a sim prior to training to establish a baseline for improvement.
        ...
        file_suffix = f'data_{data_subset}_nodes_{num_nodes}_rep_{rep}_epochs_{num_epochs}_window_{window_length}_lr_{learning_rate}_threshold_{threshold}_start_{subjects_start}_end_{subjects_end}'
