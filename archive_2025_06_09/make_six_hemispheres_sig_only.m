...
cmap = 'autumn';
% cmap = 'hot';
figure_file_name = ['h_pred_corr_sig_only_' cmap '.png'];
...
feature_table = readtable(feature_table_file,'FileType','text','delimiter','\t');
feature = feature_table.color;
alpha = 0.05/numel(feature_table.size);
is_sig = feature_table.size < alpha;
feature_min = min(feature);
feature_max = max(feature);
color_limits = [feature_min feature_max];
feature(~is_sig) = feature_min-1;
...
patch_handle_upper_left = plot_gifti_partial(surf_hemisphere_l, show_hemisphere_l, axes_upper_left, cmap, shinyness);
...
patch_handle_upper_right = plot_gifti_partial(surf_hemisphere_r, show_hemisphere_r, axes_upper_right, cmap, shinyness);
...
patch_handle_lower_left = plot_gifti_partial(surf_hemisphere_l, show_hemisphere_l, axes_lower_left, cmap, shinyness);
...
patch_handle_lower_right = plot_gifti_partial(surf_hemisphere_r, show_hemisphere_r, axes_lower_right, cmap, shinyness);
...
patch_handle_center_left = plot_gifti_partial(surf_hemisphere_l, show_hemisphere_l, axes_center, cmap, shinyness);
clim(color_limits);
% pause(delay)
...
patch_handle_center_right = plot_gifti_partial(surf_hemisphere_r, show_hemisphere_r, axes_center, cmap, shinyness);
clim(color_limits);
pause(delay)
