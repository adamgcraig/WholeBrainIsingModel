...
parser.add_argument("-w", "--windows_per_epoch", type=float, default=100, help="number of randomly selected windows on which to train between full time-series test runs")
parser.add_argument("-n", "--num_nodes", type=int, default=21, help="number of nodes to model")
parser.add_argument("-e", "--num_epochs", type=int, default=100, help="number of times to attempt a full time-series test run")
...
with torch.no_grad():
    ...
    def get_time_series_cov_mu(time_series:torch.Tensor):
        ...
        for t in range(num_times):
            cov += time_series[:,t,:,None] * time_series[:,t,None,:]
        cov = cov/num_times
    ...
    for rep in range(num_reps):    # Initialize the model and run a sim prior to training to establish a baseline for improvement.
        ...
        for epoch in range(num_epochs):
            for w in range(windows_per_epoch):
                window_length = random.randint(3, num_time_points)
                start_step = random.randint(0, num_time_points-window_length)
                end_step = start_step + window_length
                data_window = data_ts[:,start_step:end_step,:]
                data_mu_sum[:,:] = 0.0
                sim_mu_sum[:,:] = 0.0
                data_cov_sum[:,:,:] = 0.0
                sim_cov_sum[:,:,:] = 0.0
    ...
    file_suffix = f'_randi_window_data_{data_subset}_nodes_{num_nodes}_reps_{num_reps}_epochs_{num_epochs}_lr_{learning_rate}_threshold_{threshold}_start_{subjects_start}_end_{subjects_end}'
