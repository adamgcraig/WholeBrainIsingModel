...
with torch.no_grad():
    ...
    ising_model_param_string = f'nodes_{num_nodes}_window_{window_length}_lr_{scaled_learning_rate:.3f}_median_beta_{beta:.3f}'
    ...
    for batch_index in range(num_batches):
        ...
        for index_into_batch in range(current_batch_size):
            ...
            single_data_ts = ising.median_binarize_ts_data( ts=hcp.load_all_time_series_for_subject(directory_path=data_directory, subject_id=subject_id, dtype=float_type, device=device), time_dim=1 ).flatten(start_dim=0, end_dim=-2)[:num_time_points,:num_nodes]
