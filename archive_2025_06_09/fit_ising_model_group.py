...
from isingutilsslow import get_data_means_and_covs_slow
...
parser.add_argument("-v", "--num_saves", type=int, default=1000, help="number of times to save a given batch of models during training")
...
parser.add_argument("-p", "--cube_update", type=bool, default=False, help="set to True to cube the update before multiplying by the learning rate")
...
# cube_update = args.cube_update
# print(f'cube_update={cube_update}')
...
def prep_ts_data(data_directory:str, data_subset:str, num_nodes:int, window_length:int):
    ...
    data_means, data_covs = get_data_means_and_covs_slow(data_ts=data_ts, window=window_length)
...
with torch.no_grad():
    ...
    file_const_params = f'nodes_{num_nodes}_window_{window_length}_lr_{learning_rate:.3f}_threshold_{threshold_str}_beta_{beta:.3f}_group_{data_subset}_reps_{reps_per_batch}'
    for batch_index in range(num_batches):
        ...
        for super_epoch in range(num_saves):
            ...
            # if cube_update:
            #     ising_model.fit_cubed_faster(data_means=data_means, data_covs=data_covs, num_epochs=epochs_per_save, window_length=window_length, learning_rate=learning_rate)
            # else:
            ...
            if compute_fim:
                sim_fc, fim = ising_model.simulate_and_record_fc_and_fim_faster(num_steps=test_length)
            else:
