...
with torch.no_grad():
    ...
    for subject_index in range(num_subjects):
        ...
        sim_ts = torch.zeros( (num_reps, num_time_points, num_nodes), dtype=float_type, device=device )
        all_update_times = torch.zeros( (num_time_points,), dtype=int_type, device=device )
        window_update_times = torch.zeros( (windows_per_epoch+1,), dtype=int_type, device=device )
        window_update_indices = torch.zeros( (windows_per_epoch+1,), dtype=torch.long, device=device )
        total_updates = 0
        print('starting training...')
        for save_stage in range(num_saves):
            for print_stage in range(prints_per_save):
                for epoch in range(epochs_per_print):
                    all_update_times = torch.randperm(n=num_time_points, out=all_update_times)
                    window_update_times, window_update_indices = torch.sort(input=all_update_times[:windows_per_epoch+1], out=(window_update_times, window_update_indices) )
                    for window in range(windows_per_epoch):
                        window_start = window_update_times[window]
                        window_end = window_update_times[window+1]
                        window_length = window_end - window_start
                        sim_ts_window = sim_ts[:,window_start:window_end,:]
                        data_ts_window = data_ts[:,window_start:window_end,:]
                        ...
                        h += learning_rate * ( data_ts_window.mean(dim=1) - sim_ts_window.mean(dim=1) )
                        J += learning_rate * ( torch.mean(data_ts_window[:,:,:,None] * data_ts_window[:,:,None,:], dim=1) - torch.mean(sim_ts_window[:,:,:,None] * sim_ts_window[:,:,None,:], dim=1) )
                        total_updates += 1
                sim_ts, s = ising.run_batched_balanced_metropolis_sim(sim_ts=sim_ts, J=J, h=h, s=s, num_steps=num_time_points, beta=beta)
