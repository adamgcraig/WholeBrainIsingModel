...
parser.add_argument("-o", "--num_perms_group_node", type=int, default=10, help="number of permutations to use for group node-wise permutation tests")
parser.add_argument("-p", "--num_perms_group_pair", type=int, default=10, help="number of permutations to use for group node-pair-wise permutation tests")
parser.add_argument("-q", "--num_perms_individual_node", type=int, default=10, help="number of permutations to use for individual subject-wise permutation tests when we have one for each node")
parser.add_argument("-r", "--num_perms_individual_pair", type=int, default=10, help="number of permutations to use for individual subject-wise permutation tests when we have one for each node pair")
parser.add_argument("-s", "--num_perms_train_test_node", type=int, default=10, help="number of train-test splits to use for individual subject-wise cross-validation tests when we have one correlation for each node")
parser.add_argument("-t", "--num_perms_train_test_pair", type=int, default=10, help="number of train-test to use for individual subject-wise cross-validation tests when we have one correlation for each node pair")
...
parser.add_argument("-w", "--group_model_short_identifier", type=str, default='group_thresholds_31_min_0_max_3_progress', help='abbreviated name for the group model')
...
parser.add_argument("-y", "--num_epochs", type=int, default=10000, help='number of epochs for which to run Adam optimizer')
parser.add_argument("-z", "--batch_size", type=int, default=-1, help='number samples per batch in Adam optimization, default -1 means a single batch with all training data')
parser.add_argument("-0", "--learning_rate", type=float, default=0.0001, help='learning rate of Adam optimizer')
parser.add_argument("-1", "--max_num_snapshots", type=int, default=1000, help='maximum number of RMSEs and correlations to save')
parser.add_argument("-2", "--device", type=str, default='cuda:0', help='string to pass to torch.device()')
...
method_adam_str = f'adam_epochs_{num_epochs}_batchsz_{batch_size}_lr_{learning_rate:.3g}_max_snapshots_{max_num_snapshots}'
method_corr_adam_str = f'corr_{method_adam_str}'
method_rmse_adam_str = f'rmse_{method_adam_str}'
...
class ParallelLinear(torch.nn.Module):
    ...
    def __init__(self, num_linears:int, in_features:int, out_features:int, dtype, device):
        ...
        self.coeffs = torch.nn.Parameter(  torch.randn( size=(num_linears, in_features, out_features), dtype=dtype, device=device, requires_grad=True )/math.sqrt(in_features)  )
        return
    def forward(self, input:torch.Tensor):
        return torch.matmul(input, self.coeffs)

def fit_linear_model(features_train:torch.Tensor, params_train:torch.Tensor, features_test:torch.Tensor, params_test:torch.Tensor):
    in_features = features_train.size(dim=-1)
    num_linears, num_samples_train, out_features = params_train.size()
    rmse_train = torch.zeros( size=(num_snapshots, num_linears), dtype=features_train.dtype, device=features_train.device, requires_grad=False )
    rmse_test = torch.zeros( size=(num_snapshots, num_linears), dtype=features_test.dtype, device=features_test.device, requires_grad=False )
    corr_train = torch.zeros( size=(num_snapshots, num_linears), dtype=features_train.dtype, device=features_train.device, requires_grad=False )
    corr_test = torch.zeros( size=(num_snapshots, num_linears), dtype=features_test.dtype, device=features_test.device, requires_grad=False )
    local_batch_size = batch_size
    if local_batch_size == -1:
        local_batch_size = num_samples_train
    num_batches = num_samples_train//local_batch_size
    model = ParallelLinear(num_linears=num_linears, in_features=in_features, out_features=out_features, dtype=features_train.dtype, device=features_train.device)
    ...
    for epoch_index in range(num_epochs):
        ...
        if (epoch_index % epochs_per_snapshot) == 0:
            with torch.no_grad():
                ...
                snapshot_index += 1
    return model.coeffs, rmse_train, rmse_test, corr_train, corr_test
...
def get_individual_mean_state_and_fc():
        ...
        # Take the transpose so that nodes/pairs are in the stacked model (0) dimension, and subjects are in the samples (1) dimension.
        return individual_mean_state.transpose(dim0=0, dim1=1), individual_fc.transpose(dim0=0, dim1=1)
...
def get_adam_group_correlations_train_test(features:torch.Tensor, params:torch.Tensor, num_perms:int):
        ...
        corrs_test = torch.zeros( size=stat_size, dtype=features.dtype, device=features.device, requires_grad=False )
        rmses_train = torch.zeros( size=stat_size, dtype=features.dtype, device=features.device, requires_grad=False )
        rmses_test = torch.zeros( size=stat_size, dtype=features.dtype, device=features.device, requires_grad=False )
        for perm_index in range(num_perms):
            ...
            _, rmses_train[perm_index,:,:], rmses_test[perm_index,:,:], corrs_train[perm_index,:,:], corrs_test[perm_index,:,:] = fit_linear_model(features_train=features_train, params_train=params_train, features_test=features_test, params_test=params_test)
        return corrs_train, corrs_test, rmses_train, rmses_test
    
def save_adam_group_correlations_train_test(features:torch.Tensor, params:torch.Tensor, num_perms:int, feature_name:str, param_name:str, wise:str, identifer:str):
    train_corrs, test_corrs, rmses_train, rmses_test = get_adam_group_correlations_train_test(features=features, params=params, num_perms=num_perms)
    method = method_corr_adam_str
...
def get_adam_individual_correlations_train_and_test(features:torch.Tensor, params:torch.Tensor, num_perms:int, num_training_subjects:int):
    num_parts, num_subjects, _ = params.size()
    features = append_ones(features)
    stat_size = (num_perms, num_snapshots, num_parts)
    train_corrs = torch.zeros( size=stat_size, dtype=features.dtype, device=features.device, requires_grad=False )
    test_corrs = torch.zeros( size=stat_size, dtype=features.dtype, device=features.device, requires_grad=False )
    train_rmses = torch.zeros( size=stat_size, dtype=features.dtype, device=features.device, requires_grad=False )
    test_rmses = torch.zeros( size=stat_size, dtype=features.dtype, device=features.device, requires_grad=False )
    for perm_index in range(num_perms):
        permutation = torch.randperm(n=num_subjects, dtype=int_type, device=params.device)
        indices_train = permutation[:num_training_subjects]
        indices_test = permutation[num_training_subjects:]
        features_train = features[:,indices_train,:]
        features_test = features[:,indices_test,:]
        features_train, features_test = z_score_after_split_keep_1s(training=features_train, testing=features_test)
        params_train = params[:,indices_train,:]
        params_test = params[:,indices_test,:]
        params_train, params_test = z_score_after_split(training=params_train, testing=params_test)
        _, train_rmses[perm_index,:,:], test_rmses[perm_index,:,:], train_corrs[perm_index,:,:], test_corrs[perm_index,:,:] = fit_linear_model(features_train=features_train, params_train=params_train, features_test=features_test, params_test=params_test)
    return train_corrs, test_corrs, train_rmses, test_rmses
    
def save_adam_individual_correlations_train_and_test(features:torch.Tensor, params:torch.Tensor, num_perms:int, feature_name:str, param_name:str, identifer:str):
    train_corrs, test_corrs, train_rmses, test_rmses = get_adam_individual_correlations_train_and_test(features=features, params=params, num_perms=num_perms, num_training_subjects=num_training_subjects)
    wise = 'subject'
    num_testing_subjects = params.size(dim=1) - num_training_subjects
    method = method_corr_adam_str
    save_corrs(corrs=train_corrs, feature_name=feature_name, param_name=param_name, method=method, wise=wise, identifier=f'train_subjectwise_{num_training_subjects}_perms_{num_perms}_{identifer}')
    save_corrs(corrs=test_corrs, feature_name=feature_name, param_name=param_name, method=method, wise=wise, identifier=f'test_subjectwise_{num_testing_subjects}_perms_{num_perms}_{identifer}')
    method = method_rmse_adam_str
    save_corrs(corrs=train_rmses, feature_name=feature_name, param_name=param_name, method=method, wise=wise, identifier=f'train_subjectwise_{num_training_subjects}_perms_{num_perms}_{identifer}')
    save_corrs(corrs=test_rmses, feature_name=feature_name, param_name=param_name, method=method, wise=wise, identifier=f'test_subjectwise_{num_testing_subjects}_perms_{num_perms}_{identifer}')
    return 0
...
def save_group_to_individual_corrs(features:torch.Tensor, params:torch.Tensor, group_coeffs:torch.Tensor, group_features_std:torch.Tensor, group_features_mean:torch.Tensor, group_params_std:torch.Tensor, group_params_mean:torch.Tensor, feature_name:str, param_name:str, wise:str, identifier:str):
    ...
    save_corrs(corrs=corrs, feature_name=feature_name, param_name=param_name, wise=wise, method=method_corr_adam_str, identifier=f'group_to_indi_{identifier}')
    save_corrs(corrs=rmses, feature_name=feature_name, param_name=param_name, wise=wise, method=method_rmse_adam_str, identifier=f'group_to_indi_{identifier}')
    return 0
...
def save_std_mean_correlations_p_and_crit(features:torch.Tensor, params:torch.Tensor, individual_adam_corrs:torch.Tensor, num_perms:int, alpha:int, feature_name:str, param_name:str, corr_type:str, adam_feature_name:str, wise:str, identifier:str):
    ...
    adam_corr_name = f'{corr_type}_{param_name}_{adam_feature_name}'
    save_presliced_partwise_correlations_p_and_crit(features=std_features, params=individual_adam_corrs, num_perms=num_perms, alpha=alpha, feature_name=std_feature_name, param_name=adam_corr_name, wise=wise, identifer=identifier)
    save_presliced_partwise_correlations_p_and_crit(features=mean_features, params=individual_adam_corrs, num_perms=num_perms, alpha=alpha, feature_name=mean_feature_name, param_name=adam_corr_name, wise=wise, identifer=identifier)
    save_presliced_partwise_correlations_p_and_crit( features=std_params.unsqueeze(dim=-1), params=individual_adam_corrs, num_perms=num_perms, alpha=alpha, feature_name=std_param_name, param_name=adam_corr_name, wise=wise, identifer=identifier )
    save_presliced_partwise_correlations_p_and_crit( features=mean_params.unsqueeze(dim=-1), params=individual_adam_corrs, num_perms=num_perms, alpha=alpha, feature_name=mean_param_name, param_name=adam_corr_name, wise=wise, identifer=identifier )
    return 0
...
def save_corrs_feature_and_param(features:torch.Tensor, group_params:torch.Tensor, individual_params:torch.Tensor, num_perms_train_test:int, feature_name:str, param_name:str, node_or_pair_wise:str, group_identifier:str, individual_identifier:str):
    save_adam_group_correlations_train_test( features=features.mean(dim=1,keepdim=False).unsqueeze(dim=0), params=group_params, num_perms=num_perms_train_test, feature_name=f'mean_{feature_name}', param_name=f'group_{param_name}', wise=node_or_pair_wise, identifer=group_identifier )
    save_adam_individual_correlations_train_and_test(features=features, params=individual_params, num_perms=num_perms_train_test, feature_name=feature_name, param_name=param_name, identifer=f'{node_or_pair_wise}wise_{individual_identifier}')
    return 0
    
def save_corrs_node_and_pair(node_features:torch.Tensor, pair_features:torch.Tensor, group_node_params:torch.Tensor, group_pair_params:torch.Tensor, individual_node_params:torch.Tensor, individual_pair_params:torch.Tensor, node_param_name:str, pair_param_name:str, group_identifier:str, individual_identifier:str):
    save_corrs_feature_and_param(features=node_features, group_params=group_node_params, individual_params=individual_node_params, num_perms_train_test=num_perms_train_test_node, feature_name='node_features', param_name=node_param_name, node_or_pair_wise='node', group_identifier=group_identifier, individual_identifier=individual_identifier)
    save_corrs_feature_and_param(features=pair_features, group_params=group_pair_params, individual_params=individual_pair_params, num_perms_train_test=num_perms_train_test_pair, feature_name='SC', param_name=pair_param_name, node_or_pair_wise='pair', group_identifier=group_identifier, individual_identifier=individual_identifier)
    return 0

def save_corrs_group_h_and_J(node_features:torch.Tensor, sc:torch.Tensor):
     group_h, group_J = get_group_parameters()
     save_adam_group_correlations_train_test( features=node_features.mean(dim=1,keepdim=False).unsqueeze(dim=0), params=group_h, num_perms=num_perms_train_test_node, feature_name=f'mean_all', param_name=f'group_h', wise='node', identifer=group_model_short_identifier )
     save_adam_group_correlations_train_test( features=sc.mean(dim=1,keepdim=False).unsqueeze(dim=0), params=group_J, num_perms=num_perms_train_test_pair, feature_name=f'mean_sc', param_name=f'group_J', wise='pair', identifer=group_model_short_identifier )
     return 0
...
def save_corrs_individual_h_and_J(node_features:torch.Tensor, sc:torch.Tensor):
     ...
     save_adam_individual_correlations_train_and_test(features=node_features, params=individual_h, num_perms=num_perms_train_test_node, feature_name=f'all', param_name=f'h', identifer=individual_model_short_identifier )
     save_adam_individual_correlations_train_and_test(features=sc, params=individual_J, num_perms=num_perms_train_test_pair, feature_name=f'sc', param_name=f'J', identifer=individual_model_short_identifier )
     return 0

def save_corrs_h_and_J(node_features:torch.Tensor, sc:torch.Tensor):
    group_h, group_J = get_group_parameters()
    individual_h, individual_J = get_individual_parameters()
    save_corrs_node_and_pair(node_features=node_features, pair_features=sc, group_node_params=group_h, group_pair_params=group_J, individual_node_params=individual_h, individual_pair_params=individual_J, node_param_name='h', pair_param_name='J', group_identifier=group_model_short_identifier, individual_identifier=individual_model_short_identifier)
    return 0
...
def save_corrs_mean_state_and_fc(node_features:torch.Tensor, sc:torch.Tensor):
    ...
    save_corrs_node_and_pair(node_features=node_features, pair_features=sc, group_node_params=group_mean_state, group_pair_params=group_fc, individual_node_params=individual_mean_state, individual_pair_params=individual_fc, node_param_name='mean_state', pair_param_name='FC', group_identifier=group_fmri_file_name_part, individual_identifier=individual_fmri_file_name_part)
    return 0
