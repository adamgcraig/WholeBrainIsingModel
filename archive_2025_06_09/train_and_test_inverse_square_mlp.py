...
parser = argparse.ArgumentParser(description="Train and test an MLP that maps brain region structural features to an embedding that lets us predict the Ising model coupling J via an inverse-square law J[i,j] = f(SC[i,j])/||g(i) - g(j)||^2.")
...
parser.add_argument("-i", "--num_embedding_dims", type=int, default=3, help="number of dimensions in the embedding space")
...
num_embedding_dims = args.num_embedding_dims
print(f'num_embedding_dims={num_embedding_dims}')
...
# Create a class that transforms SC and node_features.
# SC just has a constant value subtracted from it so that it can take on negative values.
# We make no assumptions about the dimensions of SC.
# node_features should be 1 x num_subjects x num_nodes x num_features
# node_features gets transformed to an embedding space where it has dimensions 1 x num_subjects x num_nodes x num_embedding_dims.
# We then take pairwise distances between points using upper triangular indices.
# The resulting distances Tensor is 1 x num_subjects x num_nodes*(num_nodes-1)//2.
class StructuralFeatureSpace(torch.nn.Module):

    def __init__(self, num_nodes:int, num_node_features:int, num_embedding_dims:int, hidden_layer_width:int, num_hidden_layers:int, dtype=float_type, device='cpu'):
        super(StructuralFeatureSpace, self).__init__()
        self.sc_threshold = torch.nn.Parameter( torch.zeros( size=(1,), dtype=dtype, device=device ) )
        self.node_model = UniformMultiLayerPerceptron( num_in_features=num_node_features, num_out_features=num_embedding_dims, hidden_layer_width=hidden_layer_width, num_hidden_layers=num_hidden_layers, dtype=dtype, device=device )
        self.triu_rows, self.triu_cols = isingmodel.get_triu_indices_for_products(num_nodes=num_nodes, device=device)
    
    # Have the forward function output the entropy of the given state.
    def forward(self, sc:torch.Tensor, node_features:torch.Tensor):
        node_coords = self.node_model(node_features)
        node_sq_dists = torch.sum( torch.square(node_coords[:,:,self.triu_rows,:] - node_coords[:,:,self.triu_cols,:]), dim=-1 )
        return (sc - self.sc_threshold), node_sq_dists

def get_mlp_predictions(training_node_features:torch.Tensor, training_edge_features:torch.Tensor, training_J:torch.Tensor, validation_node_features:torch.Tensor, validation_edge_features:torch.Tensor, num_epochs:int=1000, learning_rate:float=0.001, num_hidden_layers:int=1, hidden_layer_width:int=7, num_embedding_dims:int=3):
    dtype = training_node_features.dtype
    device = training_node_features.device
    _, num_nodes, num_node_features = training_node_features.size()
    models_per_subject = training_J.size(dim=0)
    struct_model = StructuralFeatureSpace(num_nodes=num_nodes, num_node_features=num_node_features, num_embedding_dims=num_embedding_dims, hidden_layer_width=hidden_layer_width, num_hidden_layers=num_hidden_layers, dtype=dtype, device=device)
    optimizer = torch.optim.Adam( params=struct_model.parameters(), lr=learning_rate )
    # Add a singleton dimension so that the outputs of struct_model() will have the same dimensionality as training_J.
    training_SC = training_edge_features[:,:,0].unsqueeze(dim=0)
    training_node_features = training_node_features.unsqueeze(dim=0)
    # We want to train the function J[i,j] = SC[i,j]/||g(node_features[i])-g(node_features[j])||^2,
    # but we risk ending up with 0 in the denominator.
    # Instead, look at J[i,j]*||g(node_features[i])-g(node_features[j])||^2 - SC[i,j].
    for _ in range(num_epochs):
        optimizer.zero_grad()
        sc_minus, node_dists = struct_model(training_SC, training_node_features)
        loss = torch.mean( torch.square(training_J*node_dists - sc_minus) )
        loss.backward()
        optimizer.step()
        # print(f'epoch {epoch+1}, loss {loss:.3g}')
    with torch.no_grad():
        training_sc_minus, training_node_dists = struct_model(training_SC, training_node_features)
        training_J_prediction = (training_sc_minus/training_node_dists).repeat( (models_per_subject,1,1) )
        validation_sc_minus, validation_node_dists = struct_model( validation_edge_features[:,:,0].unsqueeze(dim=0), validation_node_features.unsqueeze(dim=0) )
        validation_J_prediction = (validation_sc_minus/validation_node_dists).repeat( (models_per_subject,1,1) )
    return training_J_prediction, validation_J_prediction
...
with torch.no_grad():
    ...
    edge_features, node_features = load_structural_features(input_directory=input_directory, file_name_fragment=file_name_fragment)
    ...
    features_file_name_fragment = f'{file_name_fragment}_{abs_str}_{multiply_beta_str}_epochs_{num_epochs}_lr_{learning_rate:.3g}_depth_{num_hidden_layers}_width_{hidden_layer_width}_dims_{num_embedding_dims}'
...
for rep in range(num_permutations):
    ...
    training_J_prediction, validation_J_prediction = get_mlp_predictions(training_node_features=training_node_features, training_edge_features=training_edge_features, training_J=training_J, validation_node_features=validation_node_features, validation_edge_features=validation_edge_features, num_epochs=num_epochs, learning_rate=learning_rate, num_hidden_layers=num_hidden_layers, hidden_layer_width=hidden_layer_width, num_embedding_dims=num_embedding_dims)
    with torch.no_grad():
