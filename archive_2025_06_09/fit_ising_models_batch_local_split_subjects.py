...
parser.add_argument("-s", "--num_steps", type=int, default=10, help="number of simulation steps between updates of the model parameters")
...
with torch.no_grad():
    ...
    def get_data_time_series_stats_from_single_file(data_directory:str, data_subset:str, subjects_start:int, subjects_end:int, num_nodes:int, data_threshold:float, window_length:int):
        ...
        data_ts_subset = torch.load(ts_file)[subjects_start:subjects_end,:,:,:num_nodes]
        # data_ts_subset = data_ts[subjects_start:subjects_end,:,:,:num_nodes]
        # Time dimension is second to last. Region dimension is last.
        data_std, data_mean = torch.std_mean(data_ts_subset, dim=-2, keepdim=True)
        # data_ts_norm = (data_ts_subset - data_mean)/data_std
        # data_ts_binary = torch.sign( (data_ts_subset - data_mean)/data_std - data_threshold )
        # Concatenate the 4 time series for a single subject by merging dims 1 (time series) and 2 (time).
        data_ts_flat = torch.sign( (data_ts_subset - data_mean)/data_std - data_threshold ).flatten(start_dim=1, end_dim=-2)
    ...
    file_suffix = f'data_{data_subset}_nodes_{num_nodes}_epochs_{num_epochs}_steps_{num_steps}_lr_{learning_rate}_threshold_{threshold}_start_{subjects_start}_end_{subjects_end}'
