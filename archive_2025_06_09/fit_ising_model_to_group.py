...
parser = argparse.ArgumentParser(description="Fit an Ising model to the concatenated time series of a set of subjects.' fMRI data.")
...
def get_group_fc(subject_list:list, num_nodes:int=num_nodes, data_directory:str=data_directory, epsilon:float=10e-10):
    ...
    ts_sum = torch.zeros( (num_nodes,), dtype=float_type, device=device )
    ts_product_sum = ts_sum[:,None] * ts_sum[None,:]
    num_steps = 0
    for subject_index in range(num_subjects):
        subject_id = subject_list[subject_index]
        subject_ts = ising.standardize_and_binarize_ts_data( ts=hcp.load_all_time_series_for_subject(directory_path=data_directory, subject_id=subject_id, dtype=float_type, device=device), threshold=threshold, time_dim=1 ).flatten(start_dim=0, end_dim=-2)[:,:num_nodes]
        ts_sum += torch.sum(subject_ts, dim=0)
        ts_product_sum += torch.sum( subject_ts[:,:,None] * subject_ts[:,None,:], dim=0 )
        num_steps += subject_ts.size(dim=0)
    ts_mean = ts_sum/num_steps
    ts_product_mean = ts_product_sum/num_steps
    ...
    for batch_index in range(num_batches):
        ...
        for time_point in range(num_total_time_points):
            ...
            if steps_into_subject == 0:
                ...
                single_data_ts = ising.standardize_and_binarize_ts_data( ts=hcp.load_all_time_series_for_subject(directory_path=data_directory, subject_id=subject_id, dtype=float_type, device=device), threshold=threshold, time_dim=1 ).flatten(start_dim=0, end_dim=-2)[:num_time_points_per_subject,:num_nodes]
