...
def prep_ts_data(data_directory:str, data_subset:str, num_nodes:int, window_length:int):
    ...
    print(f'binarized time series with threshold {threshold:.3g} and rearranged dimensions, time {time.time() - code_start_time:.3f}')
...
with torch.no_grad():
    ...
    file_const_params = f'nodes_{num_nodes}_window_{window_length}_lr_{learning_rate:.3f}_threshold_{threshold:.3f}_beta_{beta:.3f}_subjects_{data_subset}_reps_{reps_per_batch}'
    for batch_index in range(num_batches):
        ...
        for super_epoch in range(num_saves):
            ...
            ising_model.fit(data_means=data_means, data_covs=data_covs, num_epochs=epochs_per_save, window_length=window_length, learning_rate=learning_rate)
