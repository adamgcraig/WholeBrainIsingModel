...
with torch.no_grad():
    ...
    file_suffix = f'group_{data_subset}_parallel_{num_parallel}_sims_{num_updates_beta}_steps_{sim_length}.pt'
    # Load the file with optimized beta.
    group_ising_model_file = os.path.join(output_directory, f'ising_model_{file_suffix}')
    group_ising_model = torch.load(group_ising_model_file)
    print(f'loaded {group_ising_model_file}, time {time.time() - code_start_time:.3f}')
    previous_fc_rmse_mean = 2.0# Set to the maximum possible value.
    for update in range(num_updates):
        print(f'update {update}')
        # Do a round of fitting using Boltzmann learning.
        sim_state_mean, sim_product_mean = group_ising_model.fit(target_mean=group_state_mean, target_product_mean=group_product_mean, num_updates=1, steps_per_update=sim_length, learning_rate=learning_rate)
        # Use the means to find the FC.
        sim_group_fc = isingmodel.get_fc_binary(s_mean=sim_state_mean, s_product_mean=sim_product_mean)
    ...
    file_suffix = f'group_{data_subset}_parallel_{num_parallel}_beta_sims_{num_updates_beta}_fitting_sims_{num_updates}_steps_{sim_length}_learning_rate_{learning_rate:.3g}.pt'
    group_ising_model_file = os.path.join(output_directory, f'ising_model_{file_suffix}')
