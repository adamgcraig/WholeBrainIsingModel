...
with torch.no_grad():
    ...
    parser.add_argument("-l", "--learning_rate", type=float, default=0.0001, help="amount by which to multiply updates to the model parameters during the Euler step, to be divided by beta")
    ...
    for save_index in range(saves_per_beta_opt):
        ...
        model.fit_by_simulation_pmb_multi_learning_rate(target_state_mean=target_state_mean, target_state_product_mean=target_state_product_mean, num_updates=updates_per_save, steps_per_update=param_sim_length, learning_rate=adjusted_learning_rate, verbose=True)
