...
def get_structural_features():
    ...
    num_subjects = region_features.size(dim=0)
    one_col = torch.ones( size=(num_subjects,1), dtype=region_features.dtype, device=region_features.device )
    return torch.cat( tensors=(region_features, sc, one_col), dim=-1 )
...
def get_individual_model_parameters():
    ...
    return torch.cat( tensors=(h, J), dim=-1 )
...
def get_structure_fim():
    jacobian = get_all_lstsq_model()
    fim = get_fim()
    structure_fim = torch.matmul(  jacobian, torch.matmul( fim, torch.transpose(input=jacobian, dim0=0, dim1=1) )  )
