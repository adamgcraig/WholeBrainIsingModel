...
parser.add_argument("-s", "--sim_length", type=int, default=1000, help="number of simulation steps between Euler updates")
parser.add_argument("-u", "--num_updates", type=int, default=10, help="number of Euler updates to do")
parser.add_argument("-p", "--num_parallel", type=int, default=1200, help="number of parallel simulations to run")
...
def diffs_to_rmse(diffs:torch.Tensor):
    return torch.sqrt(  torch.mean( torch.square(diffs) )  )
...
def balanced_metropolis_sim(params:torch.Tensor, state:torch.Tensor, sim_length:int, beta:float):
    ...
    # print(J[:5,:5])
    J = J.unsqueeze(dim=0)
    state_sum = torch.zeros( (num_nodes,), dtype=float_type, device=device )
    cross_sum = torch.zeros( (num_nodes, num_nodes), dtype=float_type, device=device )
    num_flips = 0
    min_entropy_change = 10e10
    max_entropy_change = -10e10
    observed_values_sum = torch.zeros( (num_params,), dtype=float_type, device=device )
    for _ in range(sim_length):
        ...
        for node_index in range(num_nodes):
            ...
            entropy_change = -2.0*( h[:,node] + torch.sum(J[:,node,:]*state, dim=-1) )*state[:,node]*beta
            min_entropy_change = min( min_entropy_change, entropy_change.min() )
            max_entropy_change = max( max_entropy_change, entropy_change.max() )
            do_flip = threshold_choice[:,node_index] < torch.exp(entropy_change)
            num_flips += do_flip.count_nonzero()
...
with torch.no_grad():
    ...
    file_suffix = f'simple_euler_from_expected_group_{data_subset}_threshold_{threshold_str}_parallel_{num_parallel}_updates_{best_sim+1}_sim_length_{sim_length}_learning_rate_{learning_rate:.3g}_beta_{beta:.36}.pt'
