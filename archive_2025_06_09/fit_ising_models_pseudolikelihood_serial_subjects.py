...
parser.add_argument("-s", "--num_epochs", type=int, default=1000, help="number of times we iterate over the full training data set")
...
class IsingModelNegativeLogPseudoLikelihood(torch.nn.Module):
    def __init__(self, num_nodes:int, dtype=float_type, device=device, beta:float=beta):
        super(IsingModelNegativeLogPseudoLikelihood, self).__init__()
        self.beta = beta
        self.time_dim = -2
        self.node_dim = -1
        self.h = torch.nn.Parameter( torch.randn( (1, num_nodes), dtype=dtype, device=device ) )
        self.J = torch.nn.Parameter(  torch.randn( (num_nodes, num_nodes), dtype=dtype, device=device )  )
...
h = torch.zeros( size=(num_reps, num_nodes), dtype=float_type, device=device )
J = torch.zeros( size=(num_reps, num_nodes, num_nodes), dtype=float_type, device=device )
for subject_index in range(num_subjects):
    ...
    for rep in range(num_reps):
        imnlpl_fn = IsingModelNegativeLogPseudoLikelihood(num_nodes=num_nodes, dtype=float_type, device=device, beta=beta)
        ...
        print(f'subject {subject_id} ({subject_index} of {num_subjects}), rep {rep} of {num_reps}, epoch {epoch} of {num_epochs}, batch {batch} of {num_batches}, loss {loss.item():.3g}, time {time_elapsed:.3f}')
        h[rep,:] = imnlpl_fn.h.squeeze()
        J[rep,:,:] = imnlpl_fn.J
    time_elapsed = time.time() - code_start_time
