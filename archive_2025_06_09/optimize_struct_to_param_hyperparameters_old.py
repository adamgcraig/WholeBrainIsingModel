...
parser.add_argument("-k", "--max_num_hidden_layers", type=int, default=1000, help="number of hidden layers in MLP")
parser.add_argument("-l", "--max_hidden_layer_width", type=int, default=1000, help="width of each hidden layer in MLP")
parser.add_argument("-m", "--max_epochs", type=int, default=1000, help="maximum number of epochs for which to train the MLP")
parser.add_argument("-n", "--min_improvement", type=float, default=0.0001, help="minimum improvement in validation RMSE we need to see in order to keep optimizing")
...
parser.add_argument("-p", "--batch_size_increment", type=int, default=1, help="Batch sizes that we try will be multiples of this.")
parser.add_argument("-q", "--max_learning_rate_power", type=float, default=10.0, help="The smallest learning rate that we try will be 10^-this.")
parser.add_argument("-r", "--output_file_name_part", type=str, default='ising_model', help="a string to include in output file names")
...
def fit_lstsq(independent:torch.Tensor, dependent:torch.Tensor):
    num_models, num_subjects, _ = independent.size()
    ...
    indep_std, indep_mean = torch.std_mean(indep_train, dim=1, keepdim=True)
    indep_std[indep_std < min_std] = 1.0
    dep_train = dependent[:,:num_training_subjects,:]
    dep_std, dep_mean = torch.std_mean(dep_train, dim=1, keepdim=True)
    dep_std[dep_std < min_std] = 1.0
    indep_train = torch.cat(   tensors=(  (indep_train-indep_mean)/indep_std, torch.ones( size=(num_models, num_training_subjects, 1), dtype=model_dtype, device=model_device )  ), dim=-1   )
    dep_train = (dep_train - dep_mean)/dep_std
    indep_validate = torch.cat(   tensors=(  (independent[:,num_training_subjects:num_non_testing_subjects,:] - indep_mean)/indep_std, torch.ones( size=(num_models, num_validation_subjects, 1), dtype=model_dtype, device=model_device )  ), dim=-1   )
    dep_validate = (dependent[:,num_training_subjects:num_non_testing_subjects,:] - dep_mean)/dep_std
    indep_test = torch.cat(   tensors=(  (independent[:,num_non_testing_subjects:,:] - indep_mean)/indep_std, torch.ones( size=(num_models, num_testing_subjects, 1), dtype=model_dtype, device=model_device )  ), dim=-1   )
    ...
    rmse_train = isingmodellight.get_pairwise_rmse(mat1=pred_train, mat2=dep_train, dim=1)
    pred_validate = torch.matmul(indep_validate, coefficients)
    rmse_validate = isingmodellight.get_pairwise_rmse(mat1=pred_validate, mat2=dep_validate, dim=1)
    pred_test = torch.matmul(indep_test, coefficients)
    rmse_test = isingmodellight.get_pairwise_rmse(mat1=pred_test, mat2=dep_test, dim=1)
    print(f'time {time.time()-code_start_time:.3f}, least squares linear model RMSE training min {rmse_train.min():.3g}, mean {rmse_train.mean():.3g}, max {rmse_train.max():.3g}, validation min {rmse_validate.min():.3g}, mean {rmse_validate.mean():.3g}, max {rmse_validate.max():.3g}, testing min {rmse_test.min():.3g}, mean {rmse_test.mean():.3g}, max {rmse_test.max():.3g}')
...
def train_mlp(indep_train:torch.Tensor, dep_train:torch.Tensor, indep_validate:torch.Tensor, dep_validate:torch.Tensor, num_hidden_layers:int=10, hidden_layer_width:int=10, batch_size:int=670, learning_rate:float=0.001):
    ...
    num_models, num_samples, num_features = indep_train.size()
    ...
    for _ in range(num_validation_checks):
        for _ in range(epochs_per_validation):
            ...
            for _ in range(num_batches):
                ...
                opt.zero_grad()
                pred_train = model(batch_indep)
                mse = msefn(batch_dep, pred_train)
                mse.backward()
                opt.step()
        ...
        pred_validate = model(indep_validate)
        rmse_validate = isingmodellight.get_pairwise_rmse(mat1=pred_validate, mat2=dep_validate, dim=1)
        improvement = torch.max(old_rmse - rmse_validate)
        print(f'time {time.time()-code_start_time:.3f}, num epochs {total_epochs}, RMSE training min {rmse_train.min():.3g}, mean {rmse_train.mean():.3g}, max {rmse_train.max():.3g}, validation min {rmse_validate.min():.3g}, mean {rmse_validate.mean():.3g}, max {rmse_validate.max():.3g}, max validation improvement {improvement:.3g}')
    ...
    return total_epochs, rmse_train.square().mean().sqrt(), rmse_validate.square().mean().sqrt(), model
...
def do_hyperparameter_optimization(independent:torch.Tensor, dependent:torch.Tensor, max_hidden_layer_count:int, max_hidden_layer_width:int, num_perms:int):
    ...
    lr_powers = torch.arange( start=0.0, end=-1.0*(max_learning_rate_power+1.0), step=-1.0, dtype=float_type, device=model_device )
            ...
            for bs_index in range(num_batch_sizes):
                ...
                for lr_index in range(num_learning_rates):
                    ...
                    for perm_index in range(num_perms):
                        ...
                        indep_std, indep_mean = torch.std_mean(input=indep_train, dim=1, keepdim=True)
                        indep_std[indep_std  < min_std] = 1.0
                        indep_train = (indep_train - indep_mean)/indep_std
                        dep_train = dependent[:,train_indices,:]
                        dep_std, dep_mean = torch.std_mean(input=dep_train, dim=1, keepdim=True)
                        dep_std[dep_std  < min_std] = 1.0
                        dep_train = (dep_train - dep_mean)/dep_std
    ...
    dep_test = (dependent[:,num_non_testing_subjects:,:] - dep_mean)/dep_std
    min_rmse_train = rmse_train[best_hlc_index,best_hlw_index,best_bs_index,best_lr_index,:].mean()
    ...
    best_train_rmse = isingmodellight.get_pairwise_rmse(mat1=pred_train, mat2=dep_train, dim=1)
    pred_validate = best_model(indep_validate)
    best_validate_rmse = isingmodellight.get_pairwise_rmse(mat1=pred_validate, mat2=dep_validate, dim=1)
    pred_test = best_model(indep_test)
    best_test_rmse = isingmodellight.get_pairwise_rmse(mat1=pred_test, mat2=dep_test, dim=1)
    print(f'final model num training epochs {best_total_epochs}, RMSE training min {best_train_rmse.min():.3g}, mean {best_train_rmse.mean():.3g}, max {best_train_rmse.max():.3g}, validation min {best_validate_rmse.min():.3g}, mean {best_validate_rmse.mean():.3g}, max {best_validate_rmse.max():.3g}, testing min {best_test_rmse.min():.3g}, mean {best_test_rmse.mean():.3g}, max {best_test_rmse.max():.3g}')
...
def save_lstsq_results(independent:torch.Tensor, independent_name:str, dependent:torch.Tensor, dependent_name:str):
    ...
    save_and_print(obj=lstsq_rmse_train, file_name_part=f'rmse_train_{lstsq_file_part}')
    save_and_print(obj=lstsq_rmse_validate, file_name_part=f'rmse_validate_{lstsq_file_part}')
    save_and_print(obj=lstsq_rmse_test, file_name_part=f'rmse_test_{lstsq_file_part}')
...
def save_mlp_results(independent:torch.Tensor, independent_name:str, dependent:torch.Tensor, dependent_name:str, num_perms:int, max_num_hidden_layers:int, max_hidden_layer_width:int):
    ...
    save_and_print(obj=best_train_rmse, file_name_part=f'rmse_train_best_{mlp_file_part}')
    save_and_print(obj=best_validate_rmse, file_name_part=f'rmse_validate_best_{mlp_file_part}')
    save_and_print(obj=best_test_rmse, file_name_part=f'rmse_test_best_{mlp_file_part}')
    save_and_print(obj=best_model, file_name_part=f'model_{mlp_file_part}')
    return 0
...
def save_optimization_results(independent:torch.Tensor, independent_name:str, dependent:torch.Tensor, dependent_name:str, num_perms:int, max_num_hidden_layers:int, max_hidden_layer_width:int):
    save_lstsq_results(independent=independent, independent_name=independent_name, dependent=dependent, dependent_name=dependent_name)
    save_mlp_results(independent=independent, independent_name=independent_name, dependent=dependent, dependent_name=dependent_name, num_perms=num_perms, max_num_hidden_layers=max_num_hidden_layers, max_hidden_layer_width=max_hidden_layer_width)
    return 0
...
def do_node_features(region_features:torch.Tensor, param:torch.Tensor, param_name:str):
    feature_names = ['thickness', 'myelination', 'curvature', 'sulcus_depth']
    save_optimization_results(independent=region_features, independent_name='all', dependent=param, dependent_name=param_name, num_perms=num_permutations, max_num_hidden_layers=max_num_hidden_layers, max_hidden_layer_width=max_hidden_layer_width)
    num_features = region_features.size(dim=-1)
    for feature_index in range(num_features):
        save_optimization_results( independent=region_features[:,:,feature_index].unsqueeze(dim=-1), independent_name=feature_names[feature_index], dependent=param, dependent_name=param_name, num_perms=num_permutations, max_num_hidden_layers=max_num_hidden_layers, max_hidden_layer_width=max_hidden_layer_width )
    return 0

def do_node_correlation_analyses():
    region_features = get_region_features()
    do_node_features( region_features=region_features, param=get_h(), param_name='h' )
    do_node_features( region_features=region_features, param=get_mean_state(), param_name='mean_state' )
    return 0
    
def do_edge_correlation_analyses():
    sc = get_sc()
    save_optimization_results( independent=sc, independent_name='SC', dependent=get_J(), dependent_name='J', num_perms=num_permutations, max_num_hidden_layers=max_num_hidden_layers, max_hidden_layer_width=max_hidden_layer_width )
    save_optimization_results( independent=sc, independent_name='SC', dependent=get_fc(), dependent_name='FC', num_perms=num_permutations, max_num_hidden_layers=max_num_hidden_layers, max_hidden_layer_width=max_hidden_layer_width )
    return 0
