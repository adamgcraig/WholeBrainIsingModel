...
def init_from_data_mean_files(output_directory:str, data_set:str, num_folds:int, num_betas_per_target:int, random_init:bool=False, learning_rate:torch.float=0.001):
    ...
    return IsingModel(target_state_means=target_state_means, target_state_product_means=target_state_product_means, num_betas_per_target=num_betas_per_target, random_init=random_init, optimizer_name=optimizer_name, loss_fn=loss_fn, learning_rate=learning_rate)
...
file_suffix = f'{data_set}_fold_{num_folds}_betas_{num_betas_per_target}_pseudolikelihood_adam_lr_{learning_rate}.pt'
...
if os.path.exists(model_file):
    ...
    loss_fn = torch.nn.MSELoss()
    model = init_from_data_mean_files(output_directory=output_directory, data_set=data_set, num_folds=num_folds, num_betas_per_target=num_betas_per_target, random_init=random_init, learning_rate=learning_rate)
...
while model.num_param_updates < num_param_updates:
    sim_mean_state, sim_mean_state_product = model.fit_pseudolikelihood_to_stored_means_optimizer(target_ts=target_ts, num_updates=param_updates_per_save)
