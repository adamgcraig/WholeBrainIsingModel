...
with torch.no_grad():
    ...
    if type(target_flip_rate) == type(None):
        print('for target covariance...')
        beta_opt_type = 'target_cov_rmse'
        target_cov = isingmodellight.get_cov(state_mean=target_state_mean, state_product_mean=target_state_product_mean)
    else:
        print(f'for target flip rate {target_flip_rate:.3g}...')
        beta_opt_type = f'target_fr_{target_flip_rate:.3g}'
    ...
    if center_cov and use_inverse_cov:
        ...
        init_str = 'uncentered'
    base_model_file_fragment = f'{data_file_name_part}_{output_file_name_part}_init_{init_str}_reps_{models_per_subject}_{beta_opt_type}_lr_{learning_rate}_steps_{sim_length}_pupd_per_bopt_{updates_per_save}'
    num_updates_beta_total = 0
    num_updates_params_total = 0
    for beta_opt_index in range(num_beta_opts):
        print('optimizing beta...')
        if type(target_flip_rate) == type(None):
            num_beta_updates_completed = model.optimize_beta_faster(target_cov=target_cov, num_updates=num_updates_beta, num_steps=sim_length, verbose=True, min_beta=min_beta, max_beta=max_beta)
        else:
            num_beta_updates_completed = model.optimize_beta_for_flip_rate_faster(target_flip_rate=target_flip_rate, num_updates=num_updates_beta, num_steps=sim_length, verbose=True, min_beta=min_beta, max_beta=max_beta)
        ...
        for save_index in range(saves_per_beta_opt):
            ...
            model.fit_by_simulation_faster(target_state_mean=target_state_mean, target_state_product_mean=target_state_product_mean, num_updates=updates_per_save, steps_per_update=sim_length, learning_rate=learning_rate, verbose=True)
