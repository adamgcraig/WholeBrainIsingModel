...
def get_ps(ts:torch.Tensor, time_resolution:float=delta_t):
    time_dim = 0
...
def get_ps_batch(ts_batch:torch.Tensor, time_resolution:float=delta_t):
    time_dim = 1
...
class StructuralDataScaler(torch.nn.Module):
    ...
    def triu_to_rectangular(self, edge_features_triu:torch.Tensor, diagonal_fill:float=0.0):
