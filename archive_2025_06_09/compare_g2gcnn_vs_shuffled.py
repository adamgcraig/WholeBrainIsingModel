...
parser = argparse.ArgumentParser(description="Compare performance of the structure-to-Ising model GCNN vs an ensembled of the same model trained on shuffled data.")
...
parser.add_argument("-d", "--output_file_name_fragment", type=str, default='big_batch', help="another string we can incorporate into the output file names to help differentiate separate runs")
...
parser.add_argument("-f", "--patience", type=int, default=10, help="Number of epochs with no noticeable improvement in either loss before we stop and move on to the next model.")
...
parser.add_argument("-l", "--num_real", type=int, default=100, help="number of models to train and validate with correct structure-Ising model pairings")
parser.add_argument("-m", "--num_shuffled", type=int, default=100, help="number of models to train and validate with shuffled structure-Ising model pairings")
parser.add_argument("-n", "--graph_convolution_layers", type=int, default=3, help="number of pairs of a multi-layer perceptron layer followed by a graph convolution layer")
parser.add_argument("-o", "--mlp_hidden_layers", type=int, default=3, help="number of hidden layers to use in each multi-layer perceptron")
parser.add_argument("-p", "--rep_dims", type=int, default=2, help="number of nodes in each MLP hidden layer")
parser.add_argument("-q", "--batch_size", type=int, default=100, help="batch size")
...
print(f'num_real={num_real}')
num_shuffled = args.num_shuffled
print(f'num_shuffled={num_shuffled}')
graph_convolution_layers = args.graph_convolution_layers
print(f'graph_convolution_layers={graph_convolution_layers}')
...
class ShuffledStructParamsFileDataset(Dataset):
    ...
    def __getitem__(self, idx:int):
        ...
        node_features_batch, edge_features_batch, _, _ = torch.load( f=os.path.join(self.file_directory, f'example_{idx}_{self.file_suffix}.pt') )
        _, _, h_batch, J_batch = torch.load( f=os.path.join(self.file_directory, f'example_{shuffled_index}_{self.file_suffix}.pt') )
        return node_features_batch, edge_features_batch, h_batch, J_batch
...
more_complete_output_file_name_fragment = f'{output_file_name_fragment}_{file_name_fragment}_epochs_{num_epochs}_patience_{patience}_min_improvement_{min_improvement:.3g}_opt_{optimizer_name}_batch_{batch_size}_lr_{learning_rate:.3g}_rep_dims_{rep_dims}_mlp_depth_{mlp_hidden_layers}_gc_{graph_convolution_layers}'
results_file = os.path.join(output_directory, f'real_{num_real}_vs_shuffled_{num_shuffled}_{more_complete_output_file_name_fragment}.df')
real_training_rmse = torch.zeros( size=(num_real,), dtype=float_type, device=device )
real_validation_rmse = torch.zeros( size=(num_real,), dtype=float_type, device=device )
shuffled_training_rmse = torch.zeros( size=(num_shuffled,), dtype=float_type, device=device )
shuffled_validation_rmse = torch.zeros( size=(num_shuffled,), dtype=float_type, device=device )
for shuffled_index in range(num_shuffled):
    training_start_time = time.time()
    print(f'{training_start_time-code_start_time:.3f}, training shuffled model instance {shuffled_index+1} of {num_shuffled}...')
    g2gcnn_model_file = os.path.join(output_directory, f'shuffled_g2gcnn_{more_complete_output_file_name_fragment}_opt_{optimizer_name}_gconv_{graph_convolution_layers}_mlp_{mlp_hidden_layers}_rep_{rep_dims}_batch_sz_{batch_size}_lr_{learning_rate:.3g}_run_{shuffled_index}.pt')
    shuffled_training_data_set.reshuffle()
    training_mse, validation_mse = train_model(training_data_set=shuffled_training_data_set, validation_data_set=validation_data_set, g2gcnn_model_file=g2gcnn_model_file, num_epochs=num_epochs, optimizer_name=optimizer_name, rep_dims=rep_dims, mlp_hidden_layers=mlp_hidden_layers, graph_convolution_layers=graph_convolution_layers, batch_size=batch_size, learning_rate=learning_rate, save_model=save_models, patience=patience, improvement_threshold=min_improvement)
    real_training_rmse[shuffled_index] = math.sqrt(training_mse)
    real_validation_rmse[shuffled_index] = math.sqrt(validation_mse)
for real_index in range(num_real):
    training_start_time = time.time()
    print(f'{training_start_time-code_start_time:.3f}, training real model instance {real_index+1} of {num_real}...')
    g2gcnn_model_file = os.path.join(output_directory, f'g2gcnn_{more_complete_output_file_name_fragment}_opt_{optimizer_name}_gconv_{graph_convolution_layers}_mlp_{mlp_hidden_layers}_rep_{rep_dims}_batch_sz_{batch_size}_lr_{learning_rate:.3g}_run_{real_index}.pt')
    ...
    real_training_rmse[real_index] = math.sqrt(training_mse)
    real_validation_rmse[real_index] = math.sqrt(validation_mse)
torch.save( obj=(real_training_rmse, real_validation_rmse, shuffled_training_rmse, shuffled_validation_rmse), f=results_file )
