...
method_adam_str = f'adam_epochs_{num_epochs}_batchsz_{batch_size}_lr_{learning_rate:.3g}'
...
class ParallelLinear(torch.nn.Module):
    ...
    def __init__(self, num_linears:int, in_features:int, out_features:int, dtype, device):
        ...
        self.coeffs = torch.nn.Parameter(  torch.randn( size=(num_linears, in_features, out_features), dtype=dtype, device=device )/math.sqrt(in_features)  )
        return
    def forward(self, input:torch.Tensor):
        return torch.matmul(input, self.coeffs)

def fit_linear_model(features:torch.Tensor, params:torch.Tensor):
    in_features = features.size(dim=-1)
    num_linears, num_samples, out_features = params.size()
    local_batch_size = batch_size
    if local_batch_size == -1:
         local_batch_size = num_samples
    num_batches = num_samples//local_batch_size
    model = ParallelLinear(num_linears=num_linears, in_features=in_features, out_features=out_features, dtype=features.dtype, device=features.device)
    ...
    for epoch_index in range(num_epochs):
         order = torch.randperm(n=num_samples, dtype=int_type, device=features.device)
         shuffled_features = features[:,order,:]
         shuffled_params = params[:,order,:]
         batch_start = 0
...
def get_adam_group_correlations(features:torch.Tensor, params:torch.Tensor):
    coeffs = fit_linear_model(features=features, params=params)
...
def get_adam_group_correlations_p_and_crit(features:torch.Tensor, params:torch.Tensor, num_perms:int, alpha:float):
    num_thresholds, num_parts, _ = params.size()
    ...
    corrs, rmses, coeffs = get_adam_group_correlations(features=features, params=params)
    ...
    for perm_index in range(num_perms):
        perm = torch.randperm(n=num_parts, dtype=int_type, device=params.device)
        perm_corrs[:,perm_index], perm_rmses[:,perm_index], _ = get_adam_group_correlations( features=features, params=params[:,perm,:] )
...
def save_adam_group_correlations_p_and_crit(features:torch.Tensor, params:torch.Tensor, num_perms:int, alpha:float, feature_name:str, param_name:str, wise:str, identifer:str):
    corrs, p_corrs, crit_corrs, rmses, p_rmses, crit_rmses, coeffs, features_std, features_mean, params_std, params_mean = get_adam_group_correlations_p_and_crit(features=features, params=params, num_perms=num_perms, alpha=alpha)
...
def get_adam_group_correlations_train_test(features:torch.Tensor, params:torch.Tensor, num_perms:int):
        ...
        for perm_index in range(num_perms):
            ...
            coeffs = fit_linear_model(features=features_train, params=params_train)
...
def save_group_corrs(features:torch.Tensor, params:torch.Tensor, num_perms:int, alpha:float, feature_name:str, param_name:str, wise:str, identifier:str):
    ...
    coeffs, features_std, features_mean, params_std, params_mean = save_adam_group_correlations_p_and_crit(features=features, params=params, num_perms=num_perms, alpha=alpha, feature_name=feature_name, param_name=param_name, wise=wise, identifer=identifier)
    save_adam_group_correlations_train_test(features=features, params=params, num_perms=num_perms, feature_name=feature_name, param_name=param_name, wise=wise, identifer=identifier)
    return coeffs, features_std, features_mean, params_std, params_mean
...
def get_adam_individual_correlations(features:torch.Tensor, params:torch.Tensor):
    coeffs = fit_linear_model(features=features, params=params)
    predictions = torch.matmul(features, coeffs)
    corrs = isingmodellight.get_pairwise_correlation(mat1=predictions, mat2=params, epsilon=epsilon, dim=1).squeeze(dim=-1)
    rmses = isingmodellight.get_pairwise_rmse(mat1=predictions, mat2=params, dim=1).squeeze(dim=-1)
...
def get_adam_individual_correlations_p_and_crit(features:torch.Tensor, params:torch.Tensor, num_perms:int, alpha:float):
    ...
    corrs, rmses, coeffs = get_adam_individual_correlations(features=features, params=params)
    ...
    for perm_index in range(num_perms):
        perm = torch.randperm(n=num_subjects, dtype=int_type, device=params.device)
        perm_corrs[:,perm_index], perm_rmses[:,perm_index], _ = get_adam_individual_correlations(features=features, params=params[:,perm,:])
    p_corrs, crit_corrs = get_p_and_crit(corrs=corrs, perm_corrs=perm_corrs, alpha=alpha, two_way=False)
...
def save_adam_individual_correlations_p_and_crit(features:torch.Tensor, params:torch.Tensor, num_perms:int, alpha:float, feature_name:str, param_name:str, identifer:str):
    corrs, p_corrs, crit_corrs, rmses, p_rmses, crit_rmses, coeffs = get_adam_individual_correlations_p_and_crit(features=features, params=params, num_perms=num_perms, alpha=alpha)
    wise = 'subject'
    method = method_corr_adam_str
    save_corrs(corrs=corrs, feature_name=feature_name, param_name=param_name, method=method, wise=wise, identifier=identifer)
    save_p(p=p_corrs, feature_name=feature_name, param_name=param_name, method=method, wise=wise, num_perms=num_perms, identifier=identifer)
...
def get_adam_individual_correlations_train_and_test(features:torch.Tensor, params:torch.Tensor, num_perms:int, num_training_subjects:int):
    ...
    for perm_index in range(num_perms):
        ...
        params_train, params_test = z_score_after_split(training=params_train, testing=params_test)
        train_corrs[:,perm_index], train_rmses[:,perm_index], coeffs = get_adam_individual_correlations(features=features_train, params=params_train)
...
def save_individual_corrs(features:torch.Tensor, params:torch.Tensor, num_perms_p_value:int, num_perms_train_test:int, alpha:float, feature_name:str, param_name:str, wise:str, identifier:str, group_coeffs:torch.Tensor, group_features_std:torch.Tensor, group_features_mean:torch.Tensor, group_params_std:torch.Tensor, group_params_mean:torch.Tensor):
    ...
    individidual_adam_corrs, individidual_adam_rmses = save_adam_individual_correlations_p_and_crit(features=features, params=params, num_perms=num_perms_p_value, alpha=alpha, feature_name=feature_name, param_name=param_name, identifer=identifier)
    save_adam_individual_correlations_train_and_test(features=features, params=params, num_perms=num_perms_train_test, feature_name=feature_name, param_name=param_name, identifer=identifier)
    ...
    return individidual_adam_corrs, individidual_adam_rmses
...
def save_corrs_feature_and_param(features:torch.Tensor, group_params:torch.Tensor, individual_params:torch.Tensor, num_perms_group:int, num_perms_individual:int, num_perms_train_test:int, alpha_group:float, alpha_individual:float, feature_name:str, param_name:str, node_or_pair_wise:str, group_identifier:str, individual_identifier:str):
    ...
    individidual_adam_corrs, individidual_adam_rmses = save_individual_corrs(features=features, params=individual_params, num_perms_p_value=num_perms_individual, num_perms_train_test=num_perms_train_test, alpha=alpha_individual, feature_name=feature_name, param_name=param_name, wise=node_or_pair_wise, identifier=individual_identifier, group_coeffs=group_coeffs, group_features_std=group_features_std, group_features_mean=group_features_mean, group_params_std=group_params_std, group_params_mean=group_params_mean)
    save_std_mean_correlations_p_and_crit(features=features, params=individual_params, individual_adam_corrs=individidual_adam_corrs, num_perms=num_perms_group, alpha=alpha_group, feature_name=feature_name, param_name=param_name, corr_type='indi_adam_corr', adam_feature_name=feature_name, wise=node_or_pair_wise, identifier=group_identifier)
    save_std_mean_correlations_p_and_crit(features=features, params=individual_params, individual_adam_corrs=individidual_adam_rmses, num_perms=num_perms_group, alpha=alpha_group, feature_name=feature_name, param_name=param_name, corr_type='indi_adam_rmse', adam_feature_name=feature_name, wise=node_or_pair_wise, identifier=group_identifier)
    return 0
