...
with torch.no_grad():
    ...
    def load_data_ts(input_directory:str, threshold:float):
        ...
        data_ts *= 2.0
        data_ts -= 1.0
        # Permute to  nodes x subjects x ts_per_subject x times.
        data_ts = torch.permute( input=data_ts, dims=(2,0,1,3) )
        # Flatten to  nodes x (subjects * ts_per_subject * times).
        data_ts = data_ts.flatten(start_dim=-3, end_dim=-1)
        # Make a singleton batch dimension so that we do not need to rewrite some other things.
        return data_ts.unsqueeze(dim=0)
    ...
    epsilon = 0.0
    pl_params_string = f'pseudolikelihood_group_threshold_{threshold:.3g}_lr_{learning_rate:.3g}'
    ...
    beta = isingmodellight.get_linspace_beta( models_per_subject=num_betas, num_subjects=h.size(dim=0), dtype=h.dtype, device=h.device, min_beta=min_beta, max_beta=max_beta )
