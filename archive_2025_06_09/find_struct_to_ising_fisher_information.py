...
parser.add_argument("-s", "--stats_dir", type=str, default="E:\\Ising_model_results_batch", help="directory to which to write the output Fisher information matrices")
...
for subject_index in range(num_subjects):
    ...
    print( 'time', time.time() - start_time )
    features_file = hcp.get_area_features_file_path(directory_path=data_dir, subject_id=subject_id)
    node_features = ( hcp.load_matrix_from_binary(file_path=features_file, dtype=float_type, device=device).transpose(dim0=0, dim1=1) - node_mean )/node_std
    # print(node_features.device)
    print( 'loaded node features with size', node_features.size() )
    print( 'time', time.time() - start_time )
    sc_file = hcp.get_structural_connectivity_file_path(directory_path=data_dir, subject_id=subject_id)
    sc = ( hcp.load_matrix_from_binary(file_path=sc_file, dtype=float_type, device=device) - sc_mean )/sc_std
    # print(sc.device)
    print( 'loaded structural connectivity with size', sc.size() )
    print( 'time', time.time() - start_time )
    ...
    jacobian_file = os.path.join(stats_dir, f'jacobian_{model_param_string}_subject_{subject_id}_steps_{num_steps}.pt')
    ...
    fim_ising_file = os.path.join(stats_dir, f'fim_ising_{model_param_string}_subject_{subject_id}_steps_{num_steps}.pt')
    ...
    print(fim_struct.device)
    fim_struct_file = os.path.join(stats_dir, f'fim_struct2ising_{model_param_string}_subject_{subject_id}_steps_{num_steps}.pt')
    torch.save(fim_struct, fim_struct_file)
    fim_struct_sum += fim_struct
    print( 'time', time.time() - start_time )
fim_struct_mean_file = os.path.join(stats_dir, f'fim_struct2ising_{model_param_string}_{data_subset}_mean_steps_{num_steps}.pt')
torch.save(fim_struct_sum/num_subjects, fim_struct_mean_file)
print( 'done' )
print( 'time', time.time() - start_time )