...
parser.add_argument("-m", "--max_epochs", type=int, default=1000000, help="maximum number of epochs for which to train the MLP")
parser.add_argument("-n", "--min_improvement", type=float, default=0.001, help="minimum improvement in validation RMSE we need to see in order to keep optimizing")
parser.add_argument("-o", "--epochs_per_validation", type=int, default=1000, help="number of optimizer epochs between tests for improvement in validation RMSE")
...
def fit_lstsq(independent:torch.Tensor, dependent:torch.Tensor):
    ...
    rmse_train = isingmodellight.get_pairwise_rmse(mat1=pred_train, mat2=dep_train, dim=0)
    pred_validate = torch.matmul(indep_validate, coefficients)
    rmse_validate = isingmodellight.get_pairwise_rmse(mat1=pred_validate, mat2=dep_validate, dim=0)
    pred_test = torch.matmul(indep_test, coefficients)
    rmse_test = isingmodellight.get_pairwise_rmse(mat1=pred_test, mat2=dep_test, dim=0)
    ...
    return rmse_train, rmse_validate, rmse_test, coefficients
...
def do_hyperparameter_optimization(independent:torch.Tensor, dependent:torch.Tensor):
    model_dtype = independent.dtype
    model_device = independent.device
    hidden_layer_counts = torch.arange(start=0, end=max_num_hidden_layers+1, step=1, dtype=int_type, device=model_device)
    ...
    _, _, _, best_model = train_mlp(indep_train=indep_train, dep_train=dep_train, indep_validate=indep_validate, dep_validate=dep_validate, num_hidden_layers=best_num_hidden_layers, hidden_layer_width=best_hidden_layer_width, batch_size=best_batch_size, learning_rate=best_learning_rate)
    # pred_train = best_model(indep_train)
    best_train_rmse = isingmodellight.get_pairwise_rmse( mat1=best_model(indep_train), mat2=dep_train, dim=0 )
    # pred_validate = best_model(indep_validate)
    best_validate_rmse = isingmodellight.get_pairwise_rmse( mat1=best_model(indep_validate), mat2=dep_validate, dim=0 )
    # pred_test = best_model(indep_test)
    best_test_rmse = isingmodellight.get_pairwise_rmse( mat1=best_model(indep_test), mat2=dep_test, dim=0 )
...
def save_lstsq_results(independent:torch.Tensor, independent_name:str, dependent:torch.Tensor, dependent_name:str):
    rmse_train, rmse_validate, rmse_test, coefficients = fit_lstsq(independent=independent, dependent=dependent)
    lstsq_file_part = f'{independent_name}_{dependent_name}_lstsq_{output_file_name_part}'
    save_and_print(obj=rmse_train, file_name_part=f'rmse_train_{lstsq_file_part}')
    save_and_print(obj=rmse_validate, file_name_part=f'rmse_validate_{lstsq_file_part}')
    save_and_print(obj=rmse_test, file_name_part=f'rmse_test_{lstsq_file_part}')
    save_and_print(obj=coefficients, file_name_part=f'coeffs_{lstsq_file_part}')
    return 0

def save_mlp_results(independent:torch.Tensor, independent_name:str, dependent:torch.Tensor, dependent_name:str):
    rmse_train, rmse_validate, best_num_hidden_layers, best_hidden_layer_width, best_batch_size, best_learning_rate, best_train_rmse, best_validate_rmse, best_test_rmse, best_model = do_hyperparameter_optimization(independent=independent, dependent=dependent)
    all_mlp_file_part = f'{independent_name}_{dependent_name}_mlp_hidden_max_nhl_{max_num_hidden_layers}_max_hlw_{max_hidden_layer_width}_bs_incr_{batch_size_increment}_lr_e_min_{min_learning_rate_power:.3g}_max_{max_learning_rate_power:.3g}_perms_{num_permutations}_{output_file_name_part}'
    ...
    best_mlp_file_part = f'{independent_name}_{dependent_name}_mlp_hidden_num_{best_num_hidden_layers}_width_{best_hidden_layer_width}_batch_{best_batch_size}_lr_{best_learning_rate:.3g}_perms_{num_permutations}_{output_file_name_part}'
...
def save_optimization_results(independent:torch.Tensor, independent_name:str, dependent:torch.Tensor, dependent_name:str):
    ...
    save_mlp_results(independent=independent, independent_name=independent_name, dependent=dependent, dependent_name=dependent_name)
    return 0
...
def get_mean_state():
    ...
    # subject (observation dimension) x region (feature dimension)
...
def get_h():
    ...
    h = torch.mean( input=torch.load(f=model_file_name, weights_only=False).h, dim=0 )
...
def get_J():
    ...
    # subject (observation dimension) x region pair (feature dimension)
    model_file_name = os.path.join(data_directory, f'{model_file_name_part}.pt')
...
def get_fc():
    ...
    fc = isingmodellight.get_fc( state_mean=torch.mean(mean_state, dim=0), state_product_mean=torch.mean(mean_state_product, dim=0), epsilon=0.0 )[:,triu_rows,triu_cols].clone()
    print( f'time {time.time()-code_start_time:.3f}, computed FC size', fc.size() )
    return fc

def do_node_models():
    region_features = get_region_features()
    save_optimization_results( independent=region_features, independent_name='all', dependent=get_h(), dependent_name='h' )
    save_optimization_results( independent=region_features, independent_name='all', dependent=get_mean_state(), dependent_name='mean_state' )
    return 0
    
def do_edge_models():
    sc = get_sc()
    save_optimization_results( independent=sc, independent_name='SC', dependent=get_J(), dependent_name='J' )
    save_optimization_results( independent=sc, independent_name='SC', dependent=get_fc(), dependent_name='FC' )
    return 0
