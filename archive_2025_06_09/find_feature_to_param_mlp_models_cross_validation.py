...
parser.add_argument("-j", "--num_training_subjects", type=int, default=670, help="number of subjects to use when fitting the linear model. We use the remaining for testing.")
...
parser.add_argument("-l", "--learning_rate", type=float, default=0.001, help="base learning rate of Adam optimizer")
parser.add_argument("-m", "--num_node_hidden_layers", type=int, default=10, help="number of hidden layers in MLP")
parser.add_argument("-n", "--node_hidden_layer_width", type=int, default=10, help="width of each hidden layer in MLP")
parser.add_argument("-o", "--num_edge_hidden_layers", type=int, default=10, help="number of hidden layers in MLP")
parser.add_argument("-p", "--edge_hidden_layer_width", type=int, default=10, help="width of each hidden layer in MLP")
parser.add_argument("-q", "--node_batch_size", type=int, default=670, help="number of samples to use in a single backpropagation step")
parser.add_argument("-r", "--edge_batch_size", type=int, default=670, help="number of samples to use in a single backpropagation step")
parser.add_argument("-s", "--max_epochs", type=int, default=1000, help="maximum number of epochs for which to train the MLP")
parser.add_argument("-t", "--min_improvement", type=float, default=0.0001, help="minimum overall improvement in training RMSE as a fraction of SD of ground truth")
parser.add_argument("-u", "--patience", type=int, default=100, help="number of consecutive steps of improvement less than min_improvement to allow before we quit")
...
num_node_hidden_layers = args.num_node_hidden_layers
print(f'num_node_hidden_layers={num_node_hidden_layers}')
node_hidden_layer_width = args.node_hidden_layer_width
print(f'node_hidden_layer_width={node_hidden_layer_width}')
num_edge_hidden_layers = args.num_edge_hidden_layers
print(f'num_edge_hidden_layers={num_edge_hidden_layers}')
edge_hidden_layer_width = args.edge_hidden_layer_width
print(f'edge_hidden_layer_width={edge_hidden_layer_width}')
node_batch_size = args.node_batch_size
print(f'node_batch_size={node_batch_size}')
edge_batch_size = args.edge_batch_size
print(f'edge_batch_size={edge_batch_size}')
...
def save_and_print(obj:torch.Tensor, file_name_part:str):
    ...
    print( f'time {time.time()-code_start_time:.3f}, saved {file_name}, size', obj.size(), f'num NaN {num_nan}, min {obj.min():.3g} mean {obj.mean(dtype=torch.float):.3g} max {obj.max():.3g}' )
    return 0
...
def train_mlp(indep_perm_train:torch.Tensor, dep_perm_train:torch.Tensor, num_hidden_layers:int=10, hidden_layer_width:int=10, batch_size:int=670):
    model_dtype = indep_perm_train.dtype
    model_device = indep_perm_train.device
    num_models, num_samples, num_features = indep_perm_train.size()
    num_params = dep_perm_train.size(dim=-1)
    ...
    min_improvement_sd = min_improvement * torch.std(input=indep_perm_train).item()
    prediction = model(indep_perm_train)
    rmse = torch.sqrt( msefn(dep_perm_train, prediction) ).item()
    old_rmse = rmse
    improvement = 0.0
    total_epochs = 0
    num_patience = max_epochs//patience
    print(f'num batches {num_batches}, min improvement x SD {min_improvement_sd:.3g}')
    for _ in range(num_patience):
        for _ in range(patience):
            ...
            for _ in range(num_batches):
                ...
                batch_indep = indep_perm_train[:,batch_indices,:]
                batch_dep = dep_perm_train[:,batch_indices,:]
                opt.zero_grad()
                prediction = model(batch_indep)
                mse = msefn(batch_dep, prediction)
                mse.backward()
                opt.step()
                batch_start += batch_size
                batch_end += batch_size
        total_epochs += patience
        prediction = model(indep_perm_train)
        rmse = torch.sqrt( msefn(dep_perm_train, prediction) ).item()
        improvement = old_rmse - rmse
        print(f'time {time.time()-code_start_time:.3f}, num epochs {total_epochs}, RMSE {rmse:.3g}, improvement {improvement:.3g}')
        if improvement < min_improvement:
            break
        old_rmse = rmse
    print(f'time {time.time()-code_start_time:.3f}, num epochs {total_epochs}, over last {patience} epochs., improvement {improvement:.3g} was less than {min_improvement:.3g}, so we will stop.')
    return model, total_epochs
    
def do_mlp_correlation_and_validation(independent:torch.Tensor, independent_name:str, dependent:torch.Tensor, dependent_name:str, num_perms:int=10, num_hidden_layers:int=10, hidden_layer_width:int=10, batch_size:int=670):
    num_vars, num_observations, _ = dependent.size()
    num_test_subjects = num_observations - num_training_subjects
    mlp_file_part = f'{independent_name}_{dependent_name}_{model_file_name_part}_perms_{num_perms}_hidden_num_{num_edge_hidden_layers}_width_{hidden_layer_width}_batch_{batch_size}_lr_{learning_rate:.3g}'
    ...
    train_corr_file_part = f'mlp_corr_train_{num_training_subjects}_{mlp_file_part}'
    test_corr_file_part = f'mlp_corr_test_{num_test_subjects}_{mlp_file_part}'
    ...
    train_rmse_file_part = f'mlp_rmse_train_{num_training_subjects}_{mlp_file_part}'
    test_rmse_file_part = f'mlp_rmse_test_{num_test_subjects}_{mlp_file_part}'
    ...
    train_rsqd_file_part = f'mlp_rsquared_train_{num_training_subjects}_{mlp_file_part}'
    test_rsqd_file_part = f'mlp_rsquared_test_{num_test_subjects}_{mlp_file_part}'

    num_epochs = torch.zeros( size=(num_perms,), dtype=int_type, device=independent.device )

    for perm_index in range(num_perms):
        print(f'train-test split {perm_index+1} of {num_perms}...')
        ...
        mlp, num_epochs[perm_index] = train_mlp(indep_perm_train=indep_perm_train, dep_perm_train=dep_perm_train, num_hidden_layers=num_hidden_layers, hidden_layer_width=hidden_layer_width, batch_size=batch_size)
        train_pred = mlp(indep_perm_train)
        test_pred = mlp(indep_perm_test)
    ...
    save_and_print(obj=num_epochs, file_name_part=f'epochs_train_{num_training_subjects}_{mlp_file_part}')
    return 0
...
def do_node_correlation_analyses():
    ...
    do_mlp_correlation_and_validation(independent=region_features, independent_name='all', dependent=h, dependent_name='h', num_perms=num_region_permutations, num_hidden_layers=num_node_hidden_layers, hidden_layer_width=node_hidden_layer_width, batch_size=node_batch_size)
    do_mlp_correlation_and_validation(independent=region_features, independent_name='all', dependent=mean_state, dependent_name='mean_state', num_perms=num_region_permutations, num_hidden_layers=num_node_hidden_layers, hidden_layer_width=node_hidden_layer_width, batch_size=node_batch_size)
    num_features = region_features.size(dim=-1)
    for feature_index in range(num_features):
        do_mlp_correlation_and_validation( independent=region_features[:,:,feature_index].unsqueeze(dim=-1), independent_name=feature_names[feature_index], dependent=h, dependent_name='h', num_perms=num_region_permutations, num_hidden_layers=num_node_hidden_layers, hidden_layer_width=node_hidden_layer_width, batch_size=node_batch_size )
        do_mlp_correlation_and_validation( independent=region_features[:,:,feature_index].unsqueeze(dim=-1), independent_name=feature_names[feature_index], dependent=mean_state, dependent_name='mean_state', num_perms=num_region_permutations, num_hidden_layers=num_node_hidden_layers, hidden_layer_width=node_hidden_layer_width, batch_size=node_batch_size )
    return 0
    
def do_edge_correlation_analyses():
    sc = get_sc()
    do_mlp_correlation_and_validation( independent=sc, independent_name='SC', dependent=get_J(), dependent_name='J', num_perms=num_region_pair_permutations, num_hidden_layers=num_edge_hidden_layers, hidden_layer_width=edge_hidden_layer_width, batch_size=edge_batch_size )
    do_mlp_correlation_and_validation( independent=sc, independent_name='SC', dependent=get_fc(), dependent_name='FC', num_perms=num_region_pair_permutations, num_hidden_layers=num_edge_hidden_layers, hidden_layer_width=edge_hidden_layer_width, batch_size=edge_batch_size )
    return 0
