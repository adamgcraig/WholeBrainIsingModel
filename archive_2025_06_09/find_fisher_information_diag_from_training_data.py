...
# Take a batch of time series, and output one Fisher information matrix
# slow version that uses less GPU memory.
def get_fisher_info_matrix_diag_only(ts:torch.Tensor):
    print( 'ts batch size:', ts.size() )
    num_time_points = ts.size(dim=1)
    ts_augmented = torch.cat(   (  torch.ones( (1,num_time_points), dtype=ts.dtype, device=ts.device ), ts  ), dim=0   )
    num_nodes = ts_augmented.size(dim=0)
    ...
    ut_cols = ut_indices[1]
    num_pairs = ut_rows.numel()
    fim_diag = torch.zeros( (num_pairs,), dtype=ts.dtype, device=ts.device )
    # We do want to fill in the diagonal here, because we do want the variance of a product of nodes (or node*1).
    for pair_index in range(num_pairs):
        node_index_1 = ut_rows[pair_index]
        node_index_2 = ut_cols[pair_index]
        product = ts_augmented[node_index_1,:] * ts_augmented[node_index_2,:]
        mean_product = product.mean()
        fim_diag[pair_index] = torch.mean(product * product) - mean_product * mean_product
    return fim_diag
...
last_time = time.time()
fim = get_fisher_info_matrix_diag_only(data_ts)
...
last_time = time.time()
fim_file = os.path.join(output_dir, 'training_data_fim_diag.pt')
