...
hidden_layer_widths = torch.tensor(data=[1, 10, 100], dtype=int_type, device=device)
...
def iterate_over_parts_and_save(features:torch.Tensor, feature_name:str, params:torch.Tensor, param_name:str):
    ...
    for part_index in range(num_parts):
        lstsq_rmse_train[part_index], lstsq_rmse_test[part_index], lstsq_rmse_validate[part_index], best_train_rmse[part_index], best_validate_rmse[part_index], best_test_rmse[part_index] = save_optimization_results(independent=features[part_index,:,:], independent_name=f'{feature_name}_{part_index}', dependent=params[part_index,:,:], dependent_name=f'{param_name}_{part_index}', num_perms=num_permutations, max_num_hidden_layers=max_num_hidden_layers, max_hidden_layer_width=max_hidden_layer_width)
    ...
    mlp_file_name_part = f'rmse_{feature_name}_{param_name}_mlp_{output_file_name_part}_max_nhl_{max_num_hidden_layers}_max_hlw_{max_hidden_layer_width}_bs_inc_{batch_size_increment}_lr_range_e_{min_learning_rate_power:.3g}_{max_learning_rate_power:.3g}_perms_{num_permutations}'
