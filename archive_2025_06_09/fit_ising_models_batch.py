...
with torch.no_grad():
    ...
    num_nodes = 21# Choose this many brain regions at a time to model.
    anti_nan_offset = float(10**-10)
    num_steps = 50
    num_epochs = 200# number of epochs for which to train a given model
    learning_rate = 0.001
    ...
    data_fc, data_cov, data_mu = get_data_time_series_stats(data_directory=data_directory, subjects=training_subjects, time_series_per_subject=time_series_per_subject, steps_per_time_series=num_time_points, num_nodes=num_nodes, data_threshold=data_threshold, window_length=num_steps)
    ...
    file_suffix = f'_training_{num_nodes}_nodes_{num_epochs}_epochs'
