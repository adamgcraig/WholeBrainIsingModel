...
def load_structural_features(input_directory:str, file_name_fragment:str):
    ...
    # 5: x, 6: y, 7: z
    # Since x, y, and z are features of a region in the Atlas, they do not have individual variation.
    # Remove them.
    ...
    node_feature_diffs = torch.abs(node_features[:,triu_rows,:]-node_features[:,triu_cols,:])
    extended_edge_features = torch.cat( (edge_features, node_feature_diffs), dim=-1 )
...
def get_mlp_predictions(training_features:torch.Tensor, training_param:torch.Tensor, validation_features:torch.Tensor, num_epochs:int=1000, learning_rate:float=0.001, num_hidden_layers:int=1, hidden_layer_width:int=7):
    ...
    training_features_mean = torch.mean(training_features, dim=0, keepdim=True)# Take mean over individuals.
    training_features_diff = training_features - training_features_mean
    training_features_diff = training_features_diff.flatten(start_dim=0, end_dim=1)
    print( 'training features size', training_features.size() )
    training_param_means = torch.mean( training_param, dim=(0,1), keepdim=True )# Take mean over individuals and reps of individuals.
    training_param_diff = training_param - training_param_means
    training_param_diff = training_param_diff.flatten(start_dim=1, end_dim=2)
    print( 'training param size', training_param.size() )
    for _ in range(num_epochs):
        optimizer.zero_grad()
        param_diff_prediction = torch.squeeze( model(training_features_diff), dim=-1 ).unsqueeze(dim=0)
        ...
        loss = torch.mean( torch.square(param_diff_prediction - training_param_diff) )
        loss.backward()
        optimizer.step()
        # print(f'epoch {epoch+1}, loss {loss:.3g}')
    with torch.no_grad():
        # The edge means are part of the model, so we need to keep them the same for both training and validation data.
        training_param_prediction = training_param_means + torch.squeeze( model(training_features_diff), dim=-1 ).unflatten( dim=0, sizes=(1, num_training_subjects, num_regions) ).repeat( repeats=(models_per_subject,1,1) )
        num_validation_subjects = validation_features.size(dim=0)
        validation_features = (validation_features - training_features_mean).flatten(start_dim=0, end_dim=1)
        validation_param_prediction = training_param_means + torch.squeeze( model(validation_features), dim=-1 ).unflatten( dim=0, sizes=(1, num_validation_subjects, num_regions) ).repeat( repeats=(models_per_subject,1,1) )
