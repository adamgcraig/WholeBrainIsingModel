...
hidden_layer_widths = torch.tensor(data=[4, 20, 100], dtype=int_type, device=device)
...
learning_rates = torch.tensor(data=[1e-2, 1e-6, 1e-10], dtype=float_type, device=device)
...
def get_stacked_graph_layers(num_nodes:int, in_features:int, out_features:int, num_hidden_layers_f:int, num_hidden_layers_g:int, hidden_layer_width_f:int, hidden_layer_width_g:int, stack_height:int, hidden_features:int, dtype, device):
...
def train_gnn(sc_and_features_train:torch.Tensor, J_and_h_train:torch.Tensor, sc_and_features_validate:torch.Tensor, J_and_h_validate:torch.Tensor, num_graph_layers:int=10, num_graph_hidden_features:int=10, num_mlp_layers_f:int=10, num_hidden_features_f:int=10, num_mlp_layers_g:int=10, num_hidden_features_g:int=10, batch_size:int=670, learning_rate:float=0.001):
    model_dtype = sc_and_features_train.dtype
    model_device = sc_and_features_train.device
    num_samples, num_nodes, num_nodes_plus_num_features = sc_and_features_train.size()
    num_nodes_plus_num_params = J_and_h_train.size(dim=-1)
    num_batches = num_samples//batch_size
    model = get_stacked_graph_layers(num_nodes=num_nodes, in_features=num_nodes_plus_num_features-num_nodes, out_features=num_nodes_plus_num_params-num_nodes, num_hidden_layers_f=num_mlp_layers_f, num_hidden_layers_g=num_mlp_layers_g, hidden_layer_width_f=num_hidden_features_f, hidden_layer_width_g=num_hidden_features_g, stack_height=num_graph_layers, hidden_features=num_graph_hidden_features, dtype=model_dtype, device=model_device)
...
def do_hyperparameter_optimization(sc_and_features:torch.Tensor, J_and_h:torch.Tensor):
    ...
    opt_size = (num_hidden_layer_counts, num_hidden_layer_widths, num_hidden_layer_counts, num_hidden_layer_widths, num_hidden_layer_counts, num_hidden_layer_widths, num_batch_sizes, num_learning_rates, num_permutations)
    ...
    for graph_hlc_index in range(num_hidden_layer_counts):
        new_num_graph_hidden_layers = hidden_layer_counts[graph_hlc_index].item()
        for graph_hlw_index in range(num_hidden_layer_widths):
            new_graph_hidden_layer_width = hidden_layer_widths[graph_hlw_index].item()
            for f_hlc_index in range(num_hidden_layer_counts):
                new_num_f_hidden_layers = hidden_layer_counts[f_hlc_index].item()
                for f_hlw_index in range(num_hidden_layer_widths):
                    new_f_hidden_layer_width = hidden_layer_widths[f_hlw_index].item()
                    for g_hlc_index in range(num_hidden_layer_counts):
                        new_num_g_hidden_layers = hidden_layer_counts[g_hlc_index].item()
                        for g_hlw_index in range(num_hidden_layer_widths):
                            new_g_hidden_layer_width = hidden_layer_widths[g_hlw_index].item()
                            for bs_index in range(num_batch_sizes):
                                ...
                                for lr_index in range(num_learning_rates):
                                    ...
                                    for perm_index in range(num_permutations):
                                        ...
                                        print(f'graph hidden layers number {new_num_graph_hidden_layers}, width {new_graph_hidden_layer_width}, f hidden layers number {new_num_f_hidden_layers}, width {new_f_hidden_layer_width}, g hidden layers number {new_num_g_hidden_layers}, width {new_g_hidden_layer_width}, batch size {new_batch_size}, learning rate {new_learning_rate:.3g}, permutation {perm_index+1}')
                                        try:
                                            new_total_epochs, new_rmse_train, new_rmse_validate, _ = train_gnn(sc_and_features_train=sc_and_features_train, J_and_h_train=J_and_h_train, sc_and_features_validate=sc_and_features_validate, J_and_h_validate=J_and_h_validate, num_graph_layers=new_num_graph_hidden_layers, num_graph_hidden_features=new_graph_hidden_layer_width, num_mlp_layers_f=new_num_f_hidden_layers, num_hidden_features_f=new_f_hidden_layer_width, num_mlp_layers_g=new_num_g_hidden_layers, num_hidden_features_g=new_g_hidden_layer_width, batch_size=new_batch_size, learning_rate=new_learning_rate)
                                            print(f'num epochs {new_total_epochs}, RMSE training {new_rmse_train:.3g}, validation {new_rmse_validate:.3g}')
                                            total_epochs[graph_hlc_index,graph_hlw_index,f_hlc_index,f_hlw_index,g_hlc_index,g_hlw_index,bs_index,lr_index,perm_index] = new_total_epochs
                                            rmse_train[graph_hlc_index,graph_hlw_index,f_hlc_index,f_hlw_index,g_hlc_index,g_hlw_index,bs_index,lr_index,perm_index] = new_rmse_train
                                            rmse_validate[graph_hlc_index,graph_hlw_index,f_hlc_index,f_hlw_index,g_hlc_index,g_hlw_index,bs_index,lr_index,perm_index] = new_rmse_validate
    ...
    min_rmse_validate_over_g_hlw, best_g_hlw_indices = min_rmse_validate_over_bs.min(dim=-1)
    min_rmse_validate_over_g_hlc, best_g_hlc_indices = min_rmse_validate_over_g_hlw.min(dim=-1)
    min_rmse_validate_over_f_hlw, best_f_hlw_indices = min_rmse_validate_over_g_hlc.min(dim=-1)
    min_rmse_validate_over_f_hlc, best_f_hlc_indices = min_rmse_validate_over_f_hlw.min(dim=-1)
    min_rmse_validate_over_graph_hlw, best_graph_hlw_indices = min_rmse_validate_over_f_hlc.min(dim=-1)
    best_graph_hlc_index = min_rmse_validate_over_graph_hlw.argmin().item()
    best_graph_hlw_index = best_graph_hlw_indices[best_graph_hlc_index].item()
    best_f_hlc_index = best_f_hlc_indices[best_graph_hlc_index, best_graph_hlw_index].item()
    best_f_hlw_index = best_f_hlw_indices[best_graph_hlc_index, best_graph_hlw_index, best_f_hlc_index].item()
    best_g_hlc_index = best_g_hlc_indices[best_graph_hlc_index, best_graph_hlw_index, best_f_hlc_index, best_f_hlw_index].item()
    best_g_hlw_index = best_g_hlw_indices[best_graph_hlc_index, best_graph_hlw_index, best_f_hlc_index, best_f_hlw_index, best_g_hlc_index].item()
    best_bs_index = best_bs_indices[best_graph_hlc_index, best_graph_hlw_index, best_f_hlc_index, best_f_hlw_index, best_g_hlc_index, best_g_hlw_index].item()
    best_lr_index = best_lr_indices[best_graph_hlc_index, best_graph_hlw_index, best_f_hlc_index, best_f_hlw_index, best_g_hlc_index, best_g_hlw_index, best_bs_index].item()
    best_graph_num_hidden_layers = hidden_layer_counts[best_graph_hlc_index].item()
    best_graph_hidden_layer_width = hidden_layer_widths[best_graph_hlw_index].item()
    best_f_num_hidden_layers = hidden_layer_counts[best_f_hlc_index].item()
    best_f_hidden_layer_width = hidden_layer_widths[best_f_hlw_index].item()
    best_g_num_hidden_layers = hidden_layer_counts[best_g_hlc_index].item()
    best_g_hidden_layer_width = hidden_layer_widths[best_g_hlw_index].item()
    best_batch_size = batch_sizes[best_bs_index].item()
    best_learning_rate = learning_rates[best_lr_index].item()
    min_rmse_train = rmse_train[best_graph_hlc_index, best_graph_hlw_index, best_f_hlc_index, best_f_hlw_index, best_g_hlc_index, best_g_hlw_index, best_bs_index,best_lr_index,:].mean().item()
    min_rmse_validate = rmse_validate[best_graph_hlc_index, best_graph_hlw_index, best_f_hlc_index, best_f_hlw_index, best_g_hlc_index, best_g_hlw_index, best_bs_index,best_lr_index,:].mean().item()
    print(f'best hyperparameters num graph hidden layers {best_graph_num_hidden_layers}, graph hidden layer width {best_graph_hidden_layer_width}, num f() hidden layers {best_f_num_hidden_layers}, f() hidden layer width {best_f_hidden_layer_width}, num g() hidden layers {best_g_num_hidden_layers}, g() hidden layer width {best_g_hidden_layer_width}, batch size {best_batch_size}, learning rate {best_learning_rate:.3g}, RMSE training {min_rmse_train:.3g}, validation {min_rmse_validate:.3g}')
    sc_and_features_train, J_and_h_train, sc_and_features_validate, J_and_h_validate, sc_and_features_test, J_and_h_test = split_and_z_score_data(independent=sc_and_features, dependent=J_and_h)
    _, _, _, best_model = train_gnn(sc_and_features_train=sc_and_features_train, J_and_h_train=J_and_h_train, sc_and_features_validate=sc_and_features_validate, J_and_h_validate=J_and_h_validate, num_graph_layers=new_num_graph_hidden_layers, num_graph_hidden_features=new_graph_hidden_layer_width, num_mlp_layers_f=new_num_graph_hidden_layers, num_hidden_features_f=new_graph_hidden_layer_width, num_mlp_layers_g=new_num_graph_hidden_layers, num_hidden_features_g=new_graph_hidden_layer_width, batch_size=new_batch_size, learning_rate=new_learning_rate)
    ...
    return rmse_train, rmse_validate, best_graph_num_hidden_layers, best_graph_hidden_layer_width, best_f_num_hidden_layers, best_f_hidden_layer_width, best_g_num_hidden_layers, best_g_hidden_layer_width, best_batch_size, best_learning_rate, best_train_rmse, best_validate_rmse, best_test_rmse, best_model

def save_gnn_results(sc_and_features:torch.Tensor, sc_and_features_name:str, J_and_h:torch.Tensor, J_and_h_name:str):
    rmse_train, rmse_validate, best_graph_num_hidden_layers, best_graph_hidden_layer_width, best_f_num_hidden_layers, best_f_hidden_layer_width, best_g_num_hidden_layers, best_g_hidden_layer_width, best_batch_size, best_learning_rate, best_train_rmse, best_validate_rmse, best_test_rmse, best_model = do_hyperparameter_optimization(sc_and_features=sc_and_features, J_and_h=J_and_h)
    all_mlp_file_part = f'{sc_and_features_name}_{J_and_h_name}_mlp_hidden_max_nhl_{max_num_hidden_layers}_max_hlw_{max_hidden_layer_width}_bs_incr_{batch_size_increment}_lr_e_min_{min_learning_rate_power:.3g}_max_{max_learning_rate_power:.3g}_perms_{num_permutations}_{output_file_name_part}'
    ...
    best_mlp_file_part = f'{sc_and_features_name}_{J_and_h_name}_gnn_hidden_graph_num_{best_graph_num_hidden_layers}_width_{best_graph_hidden_layer_width}_f_num_{best_f_num_hidden_layers}_width_{best_f_hidden_layer_width}_g_num_{best_g_num_hidden_layers}_width_{best_g_hidden_layer_width}_batch_{best_batch_size}_lr_{best_learning_rate:.3g}_perms_{num_permutations}_{output_file_name_part}'
...
def get_sc_and_features(num_region_features:int=4):
    ...
    sc_and_features = torch.cat( tensors=(sc, region_features), dim=-1 )
...
def get_J_and_h():
    ...
    J_and_h = torch.mean(   input=torch.cat(  tensors=( model.J, torch.unsqueeze(model.h, dim=-1) ), dim=-1  ), dim=0   )
    print( f'time {time.time()-code_start_time:.3f}, loaded {model_file_name} J and h size', J_and_h.size() )
    return J_and_h
...
def get_fc_and_mean():
    ...
    fc_and_mean = torch.cat(  tensors=( fc, mean_state.unsqueeze(dim=-1) ), dim=-1  )
    return fc_and_mean

sc_and_features = get_sc_and_features()
save_gnn_results( sc_and_features=sc_and_features, sc_and_features_name='SC_and_features', J_and_h=get_J_and_h(), J_and_h_name='J_and_h' )
save_gnn_results( sc_and_features=sc_and_features, sc_and_features_name='SC_and_features', J_and_h=get_fc_and_mean(), J_and_h_name='FC_and_mean_state' )
print('done')