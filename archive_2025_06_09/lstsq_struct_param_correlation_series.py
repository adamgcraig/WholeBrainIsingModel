...
parser.add_argument("-0", "--max_updates_group", type=int, default=63000, help="last number of updates to test")
...
def save_group_corrs_h_and_J(node_features:torch.Tensor, sc:torch.Tensor, num_updates:int):
     ...
     rmses_train_node, rmses_test_node = save_lstsq_group_correlations_train_test(features=node_features, params=h, num_perms=num_perms_train_test_node, feature_name='all_mean', param_name='h_group', wise='node', identifer=group_model_short_identifier)
     rmses_train_pair, rmses_test_pair = save_lstsq_group_correlations_train_test(features=sc, params=J, num_perms=num_perms_train_test_pair, feature_name='sc_mean', param_name='J_group', wise='pair', identifer=group_model_short_identifier)
     return rmses_train_node, rmses_test_node, rmses_train_pair, rmses_test_pair
...
def save_group_corrs_h_and_J_series(node_features:torch.Tensor, sc:torch.Tensor):
    ...
    node_features = append_ones( node_features.mean(dim=0, keepdim=True) )
    sc = append_ones( sc.mean(dim=0, keepdim=True) )
    rmses_train_node = torch.zeros( size=(num_num_updates, num_thresholds, num_perms_train_test_node), dtype=node_features.dtype, device=node_features.device )
    rmses_test_node = torch.zeros_like(input=rmses_train_node)
    rmses_train_pair = torch.zeros( size=(num_num_updates, num_thresholds, num_perms_train_test_node), dtype=node_features.dtype, device=node_features.device )
    ...
    for update_index in range(num_num_updates):
        ...
        rmses_train_node[update_index,:,:], rmses_test_node[update_index,:,:], rmses_train_pair[update_index,:,:], rmses_test_pair[update_index,:,:] = save_group_corrs_h_and_J(node_features=node_features, sc=sc, num_updates=num_updates)
    method = 'lstsq_rmse'
