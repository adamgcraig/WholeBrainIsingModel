...
with torch.no_grad():
    ...
    parser.add_argument("-j", "--rand_init", action='store_true', default=False, help="Set this flag to start the simulations from randomly generated states.")
    parser.add_argument("-k", "--min_steps", type=int, default=100, help="minimum number of simulation steps before we compute the first FC RMSE")
    parser.add_argument("-l", "--max_steps", type=int, default=120000, help="maximum number of simulation steps for which we compute the last FC RMSE")
    parser.add_argument("-m", "--step_increment", type=int, default=100, help="number of simulation steps between checks of FC RMSE")
    parser.add_argument("-n", "--device", type=str, default='cuda:0', help="string to pass to torch.device()")
    ...
    if zero_h:
        ...
        zero_h_str = ''
    if rand_init:
        model.s = isingmodellight.get_random_state_like(input=model.s)
    print( f'time {time.time()-code_start_time:.3f}, loaded {model_file}' )
    
    sim_state_mean, sim_state_product_mean, flip_rate = model.simulate_and_record_means_and_flip_rate_pmb(num_steps=min_steps)
    ...
    for window_index in range(1,num_windows):
        sim_state_mean_increment, sim_state_product_mean_increment, flip_rate_increment = model.simulate_and_record_means_and_flip_rate_pmb(num_steps=step_increment)
