...
with torch.no_grad():
    ...
    output_directory = 'E:\Ising_model_results'
    ...
    def metropolis_sim(J:torch.Tensor, h:torch.Tensor, s:torch.Tensor, num_steps:int_type):
        ...
        for t in range(num_steps):
            for i in range(num_nodes):
                deltaH = 2*(  torch.dot( J_no_diag[i,:], s ) + h[i]  )*s[i]
                p_accept = torch.exp(-beta*deltaH)
                cr = choice_rand[t,i]
                s[i] *= torch.sign( cr - p_accept - (cr==p_accept).float() )# If it is equal, subtract 1 so that we accept instead of multiplying by 0.
                sim_ts[t,:] = s
        sim_ts[-1,:] = -1*sim_ts[-2,:]
        return sim_ts
    ...
    for subject_index in range(num_subjects):
        ...
        subject_id = training_subjects[0]
        ...
        print('initial FC correlation =', fc_corr)
        fc_rmse = get_triu_rmse(sim_fc, data_fc).item()
        print('initial FC RMSE =', fc_rmse)
        ...
        data_cov = torch.zeros( (num_windows, num_nodes, num_nodes), dtype=float_type, device=device )
        data_mu = torch.zeros( (num_windows, num_nodes), dtype=float_type, device=device )
        ...
        print('time epoch corr. RMSE')
        fc_corr_history = torch.zeros( (num_epochs,), dtype=float_type, device=device )
        fc_rmse_history = torch.zeros( (num_epochs,), dtype=float_type, device=device )
        for epoch in range(num_epochs):
            for window in range(num_windows):
                sim_ts = metropolis_sim(J, h, s, num_steps)
                sim_cov, sim_mu = get_time_series_cov_mu(sim_ts)
                J -= learning_rate * (sim_cov - data_cov[window,:,:])
                h -= learning_rate * (sim_mu - data_mu[window,:])
            sim_ts = metropolis_sim(J, h, s, data_length)
            sim_fc = get_fc(sim_ts)
            ...
            print( time_since_start, epoch, fc_corr.item(), fc_rmse.item() )
        
        # Save the FC correlation history, FC RMSE history, h, and J for the model.
        print('saving results...')
        fc_corr_file_name = os.path.join(output_directory, 'fc_corr', f'fc_corr_{subject_id}.pt')
        torch.save(fc_corr_history, fc_corr_file_name)
        fc_rmse_file_name = os.path.join(output_directory, 'fc_rmse', f'fc_rmse_{subject_id}.pt')
        torch.save(fc_rmse_history, fc_rmse_file_name)
        J_file_name = os.path.join(output_directory, 'J', f'J_{subject_id}.pt')
        torch.save(J, J_file_name)
        h_file_name = os.path.join(output_directory, 'h', f'h_{subject_id}.pt')
